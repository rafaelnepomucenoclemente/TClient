...
const CString CTItemDetInfo::TITEM_TOKENS[TINFOTOKEN_ITEM_COUNT] = {
	...
	CString(_T("/GemStoneLevel")),			// TINFOTOKEN_ITEM_ATTRLEVEL
...
CString CTItemDetInfo::_GetAttrString(TINFOTOKEN_ITEM eToken)
{
	...
	case TINFOTOKEN_ITEM_ATTRLEVEL:
		{
			CString test;
			//test.Format( "Level Schopností : %d + %d", m_wAttrLevel,bGems );
		//	test.Format = ("Level Schopností : %d + %d",m_wAttrLevel,bGems);
			if( 0 == m_wAttrLevel )
				return _T("");
...
case TINFOTOKEN_ITEM_GEMSTONELEVEL:
		{
			if( 0 == m_pItemTemp->m_bCanGrade )
				return _T("");

			strResult.Format( "%d", m_pItemTemp->m_bGems );
		}
		break;
		*/
			case TINFOTOKEN_ITEM_GEMSTONELEVEL:
...
DWORD CTItemInstDetInfo::_GetAttrColor(TINFOTOKEN_ITEM eToken, DWORD dwDefaultColor )
{
	...
	case TINFOTOKEN_ITEM_NEEDSLEVEL:
		...
		{
			if( m_pItemInst->GetELD() > 0 )
				return D3DCOLOR_ARGB(255, 250,243,93);
		}
		break;
