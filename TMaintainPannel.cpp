...
void CTMaintainPannel::ResetMAINTAIN( CTClientObjBase* pOBJ, DWORD dwTick, CTClientObjBase* pMainChar)
{
	...
	for( BYTE i=0; i<TMTYPE_COUNT; ++i)
	{
		...
		CTClientMaintain *pTSKILL = vTMAIN_TAIN[jj];
			...
			LPTSKILL pSkyll = CTChart::FindTSKILLTEMP(WORD(pTSKILL->m_pTSKILL->m_wSkillID));
			if(pSkyll->m_bPremIconID != 0)
			{
		
				pPotion->AddMaintain(jj,pTSKILL->m_pTSKILL->m_bPremIconID, dwCount, pTSKILL, pMainChar);
			}
			else
			{
