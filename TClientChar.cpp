...
CTClientChar::CTClientChar()
{
	...
	//CUSTOM
		m_wCharTitleID = 0;
	m_bHasCharTitle = FALSE;
	m_strCharTitle.Empty();
	//END
...
//CUSTOM
//void CTClientChar::SetCharTitle( CString strCharTitle)
//{
//	m_bHasCharTitle = TRUE;
//	m_strCharTitle = strCharTitle;
//}

void CTClientChar::SetCharTitle( WORD wCharTitleID)
{
	ResetCharTitle();

	LPTTITLE pTitle = CTChart::FindTTITLETEMP(wCharTitleID);

	if ( pTitle && pTitle->m_wTitleID != 0 )
	{
		m_wCharTitleID = wCharTitleID;
		m_bHasCharTitle = TRUE;
		m_strCharTitle = pTitle->m_strTitle;
	}
}

WORD CTClientChar::GetCharTitle()
{
	if( !m_bHasCharTitle )
		return NULL;

	return m_wCharTitleID;
}

void CTClientChar::ResetCharTitle()
{
	m_bHasCharTitle = FALSE;
	m_wCharTitleID = 0;
	m_strCharTitle = CString("");
}
//END
