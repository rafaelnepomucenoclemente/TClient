...
BOOL CALLBACK NPGameMonCallback(DWORD dwMsg, DWORD dwArg)
{
	...
	case NPGAMEMON_INIT_ERROR:
		theApp.GetMainWnd()->PostMessage( WM_QUIT ); //vypÌn·nÌ hry ok?
	...
	case NPGAMEMON_CHECK_CSAUTH2:
		...
		{
			MessageBox( NULL, "apdejt","xd",MB_OK );
...
BOOL CTClientApp::InitInstance()
{
	...
	else	// ±π≥ª. ¥‹æÓ¥‹¿ß∑Œ ¿⁄∏£¥Ÿ∫∏¥œ ±‰ ¥‹æÓ ¿œ ∂ß ≥ π´ ∏π¿Ã ¿ﬂ∑¡º≠ æ∆∏ß¥‰¡ˆ æ ¿Ω.
	{
		TComponent::m_bUseWordBreak = FALSE;
	}
    //return 0;
	...
	MD5 context(file);
		...
		if (strRESULT != "c7bc1458b8e13f1682f5a2ad52edee44")
		{
			//AfxMessageBox(strRESULT, MB_OK, NULL);

			AfxMessageBox("TCD Files were changed.", MB_OK, NULL);
			exit(0);
		}
...
int CTClientApp::ExitInstance()
{
	...
	// ∞‘¿”∞°µÂ.
	if (m_pNpgl)//if( CTNationOption::RUSSIA && m_pNpgl )
	{
		delete m_pNpgl;
		m_pNpgl = NULL;
	}
...
#define REG_SETTINGS_RESET_IDENTITY		(0x00000001)		// ±‚¡∏ ∞™¿ª ¥Ÿ∏• ∞™¿∏∑Œ ºº∆√«œ∏È µ∆˙∆Æ∑Œ ∏Æº¬µ»¥Ÿ. ( 0¿ª ¡¶ø‹«— ∞™ )
...
BOOL CTClientApp::HackShield_Init()
{
	...
	dwOption = AHNHS_CHKOPT_ALL | AHNHS_CHKOPT_STANDALONE | AHNHS_DISPLAY_HACKSHIELD_TRAYICON
		| AHNHS_CHKOPT_UPDATED_FILE_CHECK | AHNHS_CHKOPT_LOCAL_MEMORY_PROTECTION | AHNHS_CHKOPT_PROTECT_D3DX;
