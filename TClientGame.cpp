...
DWORD CTClientGame::m_vTUSELESS[14] = {
	ID_CTRLINST_ICON_USESEL_1,
	ID_CTRLINST_ICON_USESEL_2,
	ID_CTRLINST_ICON_USESEL_3,
	ID_CTRLINST_ICON_USESEL_4,
	ID_CTRLINST_ICON_USESEL_5,
	ID_CTRLINST_ICON_USESEL_6,
	ID_CTRLINST_ICON_USESEL_7,
	ID_CTRLINST_ICON_USESEL_8,
	ID_CTRLINST_ICON_USESEL_9,
	ID_CTRLINST_ICON_USESEL_10,
	ID_CTRLINST_ICON_USESEL_11,
	ID_CTRLINST_ICON_USESEL_12,
	ID_CTRLINST_ICON_USESEL_13,
	ID_CTRLINST_ICON_USESEL_14}; //fix bad ID!
...
HRESULT CTClientGame::Render( DWORD dwTickCount)
{
	...
	// GameGuard
	if (CNPGameLib* pNpgl = theApp.m_pNpgl)
	{
		...
		if (dwResult != NPGAMEMON_SUCCESS)
		{
			...
			MessageBox(NULL, CTChart::Format( TSTR_NPGG_ERROR ), strCaption, MB_OK);
			exit(0);
		}
	}
	// GameGuard
	...
	CalcBOWTick(dwTickCount);
	CalcLOTTick(dwTickCount);
...
if( pTCHAR->m_bHasCharTitle && !pTCHAR->m_strCharTitle.IsEmpty() )
					strNAME = CTChart::Format( TSTR_FMT_FIELDRANKNAME, pTCHAR->m_strCharTitle, strNAME);
...
void CTClientGame::CalcBREATH( DWORD dwTick)
{
	...
	if( pTOBJ->m_bDive && dwBreath < dwTick || pTOBJ->m_bDive  && (m_vMAP.m_wMapID == 2200 || m_vMAP.m_wMapID == 2201 || m_vMAP.m_wMapID == 2202 || m_vMAP.m_wMapID == 2203 || m_vMAP.m_wMapID == 2204 || m_vMAP.m_wMapID == 3000))
	{
		...
		for(; itPATHMON != end ; ++itPATHMON )
		{
				...
				fDist - fAB > 0.125f )
			{
				...
				if( index >= 0 && index < ADJACENCY_COUNT )
				{
					...
					// ∞°¡ﬂƒ°ø° ¿««ÿº≠ LeftøÕ Right¿« ø¿«¡º¬¿ª ¡∂¿˝«œ¿⁄.
					INT nLeftCount = 5;
					INT nRightCount = 5;

					if( vCROSS.y >= 0.0f ) //±‚¡ÿ ∫§≈Õø°º≠ ø¿∏•¬ ø° ¿÷¿∏∏È ≈©∑ŒΩ∫ ∞·∞˙∫§≈Õ¿« Y√‡¿Ã æÁºˆ¥Ÿ.
					{
						nLeftCount -= 1;
						nRightCount += 1;
					}
					else if( vCROSS.y < 0.0f ) //±‚¡ÿ ∫§≈Õø°º≠ øﬁ¬ ø° ¿÷¿∏∏È ≈©∑ŒΩ∫ ∞·∞˙∫§≈Õ¿« Y√‡¿Ã ¿Ωºˆ¥Ÿ.
					{
						nLeftCount += 1;
						nRightCount -= 1;
					}
									...
									DWORD dwTick)
{
	...
	if(bROAM || bOnSDT_RUNAWAY)
	{
		...
		if(!pTARGET || bOnSDT_RUNAWAY)
		{
			...
			// ∞¯∆˜µπˆ«¡±∏«ˆ
			if( bOnSDT_RUNAWAY ) // ∞¯∆˜»ø∞˙∞° æ¯¿∏∏È ±◊ ≈∏∞Ÿ¿ª ¥ÎªÛ¿∏∑Œ.
			{
...
ON_GM_COMMAND(GM_SELECT_TITLE)
	...
	ON_GM_COMMAND(GM_HOTCHAR)
		ON_GM_COMMAND(GM_HOTPOST)
		ON_GM_COMMAND(GM_HOTWAR)
	ON_GM_COMMAND(GM_HOTCABINET)
		ON_GM_COMMAND(GM_HOTCOMM)
	...
	ON_GM_COMMAND(GM_GUILD_TERRITORY)
		ON_GM_COMMAND(GM_GUILD_HOTTERRITORY)
...
//	ON_GM_COMMAND(GM_GMP_VOLUNTEER_SKYGARDEN)
...
//	ON_GM_COMMAND( GM_TEXTOOL_COMPO )
	...
	ON_GM_COMMAND(GM_LOTTERY_START)
	ON_GM_COMMAND(GM_LOTTERY_END)
...
int CTClientGame::OnSessionMsg( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	ON_TRECEIVE(CS_UPDATELOT_REQ)
	ON_TRECEIVE(CS_GETLREWARD_REQ)
	ON_TRECEIVE(CS_WLLOT_REQ)
...
// 	ON_TRECEIVE(CS_LOT_RESET)
	...
	// Duel message
		ON_TRECEIVE(CS_CHARTITLE_ACK)
	ON_TRECEIVE(CS_CHGTITLE_ACK)
	ON_TRECEIVE(CS_TITLELIST_ACK)
	ON_TRECEIVE(CS_TITLEUPDATE_ACK)
	...
	ON_TRECEIVE(CS_STOPTHECLOCK_ACK)
		ON_TRECEIVE(CS_KILLSTATUE_REQ)
	ON_TRECEIVE(CS_BOWTICK_ACK) //custom
	ON_TRECEIVE(CS_BOWANNOUNCE_ACK)
...
void CTClientGame::BeginCharge( DWORD dwTick, TCHARGETYPE eCHARGETYPE)
{
	if(!m_bLOOPCHARGE)
	{
		...
		m_vTGAUGE[TGAUGE_CHARGE]->ShowComponent(TRUE);
				int x,y;
CPoint poska;
m_vTGAUGE[TGAUGE_CHARGE]->GetComponentPos(&poska);
CString pos;
pos.Format("ID : %d",poska.y);
...
BYTE CTClientGame::CloseTopFrame()
{
	...
	else if( bFrameID == TFRAME_TOURNAMENT_GAGE )
	{
		...
		else if(bFrameID == TFRAME_AVSTATUS)
	{
		return (BYTE)T_INVALID;
	}
		else if(bFrameID == TFRAME_BOWALARM)
	{
		return (BYTE)T_INVALID;
	}
		else if(bFrameID == TFRAME_BOWREV)
		{
			return (BYTE)T_INVALID;
		}
		else if(bFrameID == TFRAME_BOWSTATUS)
	{
		return (BYTE)T_INVALID;
	}
		else if(bFrameID == TFRAME_LOTTERY)
		{
			return (BYTE)T_INVALID;
		}
			else if(bFrameID == TFRAME_LOTWIN)
		{
			return (BYTE)T_INVALID;
		}
...
void CTClientGame::OnActivateKEY( WORD wSCAN)
{
	...
	case TKEY_HOTKEY_F1			: 
        ...
        pMainUI->m_pCurCount->m_strText =  CTChart::Format( TSTR_FMT_NUMBER, 1);
	...
	case TKEY_HOTKEY_F2			: 
		...
		CTMainUI* pMainUI = static_cast<CTMainUI*>( CTClientGame::GetInstance()->GetFrame( TFRAME_MAIN ) );
        pMainUI->m_pCurCount->m_strText =  CTChart::Format( TSTR_FMT_NUMBER, 2);
	...
	case TKEY_HOTKEY_F3			: 
		...
		CTMainUI* pMainUI = static_cast<CTMainUI*>( CTClientGame::GetInstance()->GetFrame( TFRAME_MAIN ) );
        pMainUI->m_pCurCount->m_strText =  CTChart::Format( TSTR_FMT_NUMBER, 3);
		}
		break;
	case TKEY_HOTKEY_F4			: 
		{
		CTMainUI* pMainUI = static_cast<CTMainUI*>( CTClientGame::GetInstance()->GetFrame( TFRAME_MAIN ) );
        pMainUI->m_pCurCount->m_strText =  CTChart::Format( TSTR_FMT_NUMBER, 4);
	...
	case TKEY_HOTKEY_F5			: 
		{
		CTMainUI* pMainUI = static_cast<CTMainUI*>( CTClientGame::GetInstance()->GetFrame( TFRAME_MAIN ) );
        pMainUI->m_pCurCount->m_strText =  CTChart::Format( TSTR_FMT_NUMBER, 5);
	...
	case TKEY_HOTKEY_F6			: 
		{
		CTMainUI* pMainUI = static_cast<CTMainUI*>( CTClientGame::GetInstance()->GetFrame( TFRAME_MAIN ) );
        pMainUI->m_pCurCount->m_strText =  CTChart::Format( TSTR_FMT_NUMBER, 6);
		SelectHotKey(THOTKEYBASE_DEF, 9);
		SelectHotKey(THOTKEYBASE_SUB1, 10);
		}
		break;
	case TKEY_HOTKEY_F7			: 
		{
...
void CTClientGame::OnRegionChanged()
{
	...
	pMainUI->m_Region->m_strText = m_pMainChar->m_pTREGION->m_strNAME;
		...
		case TFRAME_INVEN_5		:
			{
				...
				if(pTINVEN)
				{
					...
					if( pTITEM && m_pSESSION )
						...
						else if( pTITEM->GetTITEM()->m_bType == 21 )
						{

								DWORD m_dwCmdParam = pTITEM->m_wItemID;
							CString msg;
							msg = "PokuÔ pouûijeö sedlo, peËeù bude odstranÏna. PokraËovat?";
							m_pMainWnd->MessageBoxYesNo(
								msg,
								TSTR_YES,
								TSTR_NO,
								TCOMMAND( GM_USE_SADDLE, MAKELONG( GetInvenID(bTFRAME), j ) ),
								GM_CLOSE_MSGBOX,
								0,
								FALSE );
						}
		...
		case TFRAME_CHAR_INFO	:
			{
				...
				else
				{
					...
					for( BYTE n=0 ; n < MAX_SUBINVEN ; ++n )
					{
						...
						{
							WORD wInvenPos = GetBlankInvenPosExceptInven( n ); // ¿⁄Ω≈¿ª ¡¶ø‹«— ∞°πÊ¿« ∫Ûƒ≠¿ª √£æ∆æﬂ «—¥Ÿ.

							BYTE ToInven = HIBYTE(wInvenPos); // ¿Œ∫•
							BYTE ToSlot = LOBYTE(wInvenPos); // ΩΩ∑‘
...
BYTE CTClientGame::PannelHitTest( CPoint point)
{
	...
	for( i=0; i<TFACEIMG_COUNT; i++)
		...
		//	for( i=0; i<TNEWFACEIMG_COUNT; i++)
	//	if(m_vNEWFACE[i]->HitTest(point))
	//		switch(i)
	//		{
	//		case TNEWFACEIMG_PLAYER		: return TPANNEL_PLAYER;
	//		}
	...
	if(CheckDEFMSG( pTATTACK, pTDEFEND, bBroadCast))
	{
		...
		if(bDEFEND)
		{
			...
			else
			{
				if( pTDEFEND )
				{
					// ¿Ã µ∆Ê¥ı¥¬ m_pMainChar¿Ã∞≈≥™ ≥ª∞° »£Ω∫∆Æ ¡ﬂ¿Œ ∏ÛΩ∫≈Õ, º“»Øºˆ¿œ ºˆ æ¯¥Ÿ.
...
void CTClientGame::ShotArrow( CTClientObjBase *pTOBJ, LPTSKILLDATA pSKILLDATA, LPTSKILL pTSKILL)
{
	...
	if( pTITEM )
	{
		...
		if( pTVISUAL )
		{
			...
			{
				TRACE("Warning(CTClientGame::ShotArrow): ∞¯∞›¿⁄(TYPE:%u ID:%u)∞° πﬂªÁ¡ˆ¡°¿∏∑Œ ªÁøÎ«“ "
					"««∫ø¡°(0x%x)¿ª √£¿ª ºˆ æ¯æÓ ≈∏∞Ÿø° ¥Î«œø© Ω∫≈≥(%s)¿ª ªÁøÎ«“ ºˆ æ¯Ω¿¥œ¥Ÿ.\n",
					pTOBJ->m_bType, 
					pTOBJ->m_dwID, 
...
*/
		...
		case TFRAME_PORTAL_SHOP		:
		case TFRAME_PORTAL_SHOP_NEW	:
		case TFRAME_AVSTATUS:
		case TFRAME_AVWIN	:
		//case TFRAME_VETERAN :
		case TFRAME_HOTSTART:  
		case TFRAME_BOWALARM:
		case TFRAME_TITLE:   
...
void CTClientGame::ResetTargetOBJ( CTClientObjBase *pTARGET)
{
			...
			m_BUUU = pMain->FindKid(ID_CTRLINST_BU);
AddKid(m_BUUU);
RemoveKid(m_BUUU);
AddKid(pMain->m_BU);
RemoveKid(pMain->m_BU);
AddKid(pMain->m_Povest);
RemoveKid(pMain->m_Povest);
AddKid(pMain->m_Channel);
RemoveKid(pMain->m_Channel);
	...
	case TSKILLTARGET_ALLIANCE		:
		{
			...
			// TARGET¿Ã æ¯∞≈≥™ ¿˚±π¿Ã∏È Ω∫≈≥Ω√¿¸¿« ¥ÎªÛ¿ª ¿⁄Ω≈¿∏∑Œ «—¥Ÿ.
			// TARGET¿Ã µø∏Õ¿Ã∏È PC,MON,RECALL,SELF∞° æ∆¥— ∞ÊøÏ(SWITCH≥™ GATE∞∞¿∫) Ω∫≈≥Ω√¿¸¿« ¥ÎªÛ¿ª ¿⁄Ω≈¿∏∑Œ «—¥Ÿ.
			// TARGET¿Ã ∆Í¿Ã∏È ¥ÎªÛ¿ª ¿⁄Ω≈¿∏∑Œ «—¥Ÿ.
			if( !pTARGET ||
				!bAlliance ||
...
{
	CTMainUI* pMain = static_cast<CTMainUI*>( m_vTFRAME[TFRAME_MAIN] );
	AddKid(pMain->m_Region);
	RemoveKid(pMain->m_Region);
	AddKid(pMain->m_BU);
	RemoveKid(pMain->m_BU);
	AddKid(pMain->m_Povest);
	RemoveKid(pMain->m_Povest);
	...
	BYTE bHUD = GetHUDOption();
		...
		m_pTMinimap->pHPGauge->SetGauge(
				m_pMainChar->m_dwHP,
				m_pMainChar->m_dwMaxHP,
				bContinue);

		m_pTMinimap->pMPGauge->SetGauge(
				m_pMainChar->m_dwMP,
				m_pMainChar->m_dwMaxMP,
				bContinue);

		m_pTMinimap->pMPTEXT->m_strText = CTChart::Format( TSTR_FMT_LEVEL,
		m_pMainChar->m_dwHP);
			m_pTMinimap->pMPTEXT2->m_strText = CTChart::Format( TSTR_FMT_LEVEL,
		m_pMainChar->m_dwMP);
	...
	m_vTTEXT[TTEXT_PLAYER_MP]->m_strText = CTChart::Format( TSTR_FMT_GAUGE,
		m_pMainChar->m_dwMP,
		m_pMainChar->m_dwMaxMP);
     RemoveKid(m_vTGAUGE[TGAUGE_PLAYER_EXP]);
	...
	m_vTHUDGAUGE[THUD_GAUGE_MY_HP]->ShowComponent(FALSE);

	m_vTHUDGAUGE[THUD_GAUGE_MY_MP]->ShowComponent(FALSE);
	m_vTHUDBASE[THUD_BASE_MY_HP]->ShowComponent(FALSE);
	m_vTHUDBASE[THUD_BASE_MY_MP]->ShowComponent(FALSE);
	m_vTHUDTEXT[THUD_TEXT_MY_HP]->ShowComponent(FALSE);
	m_vTHUDTEXT[THUD_TEXT_MY_MP]->ShowComponent(FALSE);
		...
		case OT_PC		:
			{
				...
				m_vTFACEIMG[TFACEIMG_TARGETPC]->SetCurImage(((CTClientChar *) pTARGET)->m_bClassID+10);
...
void CTClientGame::ResetHUDTargetINFO( BYTE bContinue)
{
	...
	m_vTHUDBASE[THUD_BASE_OP_HP]->ShowComponent(FALSE);
	m_vTHUDTEXT[THUD_TEXT_OP_HP]->ShowComponent(FALSE);
	m_vTHUDGAUGE[THUD_GAUGE_OP_HP]->ShowComponent(FALSE);
...
*/
	...
	for( BYTE i=0; i<MAX_PARTY; i++)
	{
		if( i < bCount )
		{
		m_vTFACEIMG[vFACEIMG[i]]->SetCurImage(m_vTPARTY[i]->m_bClass+10);
...
CTMainUI* pMainUI = static_cast<CTMainUI*>(m_vTFRAME[TFRAME_MAIN]);
	if( bCount )
	{
		...
		{
		case PT_FREE	: pMainUI->m_pPartyIT->m_strText = CTChart::LoadString( TSTR_FMT_PARTY_ITEM_FREE);		break;
		case PT_HUNTER	: pMainUI->m_pPartyIT->m_strText = CTChart::LoadString( TSTR_FMT_PARTY_ITEM_HUNTER);	break;
		case PT_LOTTERY	: pMainUI->m_pPartyIT->m_strText = CTChart::LoadString( TSTR_FMT_PARTY_ITEM_LOTTERY);	break;
		case PT_CHIEF	: pMainUI->m_pPartyIT->m_strText = CTChart::LoadString( TSTR_FMT_PARTY_ITEM_CHIEF);	break;
		case PT_ORDER	: pMainUI->m_pPartyIT->m_strText = CTChart::LoadString( TSTR_FMT_PARTY_ITEM_SEQ);	break;
		default			: pMainUI->m_pPartyIT->m_strText.Empty();
		}

		if( !pMainUI->m_pPartyIT->m_strText.IsEmpty() )
		{
			pMainUI->m_pPartyIT->EnableComponent(m_pMainChar->m_dwChiefID == m_pMainChar->m_dwID);
			pMainUI->m_pPartyIT->ShowComponent(TRUE);
		}
	}
	else
		pMainUI->m_pPartyIT->ShowComponent(FALSE);
...
void CTClientGame::ResetInvenUI()
{
	...
	if( m_bLuckyID != 255)
	{
		...
		pMainUI->m_pLuckyText->m_strText = strTEXT;
		//m_vTTEXT[TTEXT_PLAYER_CRON]->m_strText = strTEXT;
			
		//pTGaugePannel->m_pLUCKY3->m_strText = CTChart::Format( TSTR_FMT_LUCKNUMBER, m_bLuckyID);
	//	pTGaugePannel->m_pLUCKY->ShowComponent(TRUE);
	}

	//	pTGaugePannel->m_pLUCKY->ShowComponent(FALSE);

//	m_vTTEXT[TTEXT_PLAYER_CRON]->m_strText = strEDIT;
			...
			case TFRAME_CHAR_INFO	:
				{
						...
						TMAINSLOTID_INVEN == CTMainUI::ToSlotType(bSlotID) )
					{
						...
						TMAINSLOTID_INVEN == CTMainUI::ToSlotType(m_vDRAG.m_bSlotID) )
					{
						// ƒ≥∏Ø≈Õ¡§∫∏√¢ø°º≠ ∞°πÊ¿ª ¥Ÿ∏• ∞°πÊΩΩ∑‘¿∏∑Œ ø≈±Ê∂ß
						DropItem_MainUIToMainUI(nFlags, bTargetUI, bSlotID);
					}
					else
					{
						// ¿Œ∫•ø°º≠ ¿Œ∫•¿∏∑Œ æ∆¿Ã≈€¿ª ø≈±Ê∂ß
...
void CTClientGame::DropItem_InvenToAuctionReg( UINT nFlags, BYTE bTargetUI, BYTE bSlotID )
{
	...
	if( pTInven )
	{
		...
		if( pRegFrame->IsVisible() )
		{
			if( pItem &&
				pItem->GetWrap() || 
				pItem->GetTITEM()->m_bCanTrade & 1 ) // ∞≈∑°∞°¥… æ∆¿Ã≈€
			{
...
void CTClientGame::DropItem_MainUIToDefault( UINT nFlags, BYTE bTargetUI, BYTE bSlotID, CPoint ptDrag )
{
	...
	if( m_vDRAG.m_dwParam == -1 ) // ¿Ã πÆ¿Â∞˙ ∞¸∑√µ» ∑Œ¡˜ : CTRSCSDlg::OnBeginDrag
		return ;
...
void CTClientGame::BuildPCMENU( CTClientChar *pTCHAR)
{
	...
	for( BYTE i=0; i< dwCount; i++)
	{
		...
		case GM_CORPS_SETCOMMANDER	:
			if( m_bTournamentWatching || m_vMAP.m_wMapID == 3000 || !wSquadID || m_pMainChar->m_dwChiefID != m_pMainChar->m_dwID ||
		...
		case GM_FRIEND_REQ			:
			{
				if( IsMiniGameBattle() || m_vMAP.m_wMapID == 3000 || m_bTournamentWatching || bTargetChallengeGame || !bGuildFriendAlliance )
					bADD = FALSE;
				else
				{
					CTMsgGroupList* pList = m_pMessengerBase->GetMsgGroupList();
		...
		case GM_SHOW_INVITE_TACTICS:
			if( IsMiniGameBattle() || m_vMAP.m_wMapID == 3000 || m_bTournamentWatching || bTargetChallengeGame || !bBattleAlliacne || !m_pMainChar->m_dwGuildID || 
							 ...
							 BYTE bSlotID)
{
	...
	if( pTITEM->GetTITEM()->m_bKind != IK_REVIVAL )
	{
		...
		if(!pTPET && pTITEM->GetTITEM()->m_bKind == IK_WHIP)
		{
			...
			CTChart::LoadString( TSTR_INFO_TITLE),
			m_pMainChar->GetName(), 
			"You only can use your Horse Whip on your Mount !",
			TCOLOR_ERROR,
			TCHAT_FLAG_INFO);
*/
			return;
	...
	case IK_NPCCALL:
		{
			...
			{			
				...
				switch( pTNPC->m_bNPCType )
				{
					...
					case TNPC_DIVINER: //¡°º˙∞°(º“øÔ∏ﬁ¿Ã∆Æ)
						{
...
*/
			...
			if(CTClientItem::IsLocked(pTITEM->GetTITEM()->m_wDelayGroupID))
			{
				...
				CTChart::LoadString( TSTR_INFO_TITLE),
				m_pMainChar->GetName(), 
				"Schopnost jeötÏ nenÌ p¯ipravena.",
				TCOLOR_ERROR,
				TCHAT_FLAG_INFO);
			...
			if(pTPET && pTITEM->GetTITEM()->m_bKind != IK_WHIP)
			{
				...
				pTPET->TakeDown();
				SendCS_PETRIDING_REQ(pTRECALL->m_dwID,0);
...
void CTClientGame::ResetWhip()
{

		MSG msg;

		SetTimer(NULL, 0, 10000, 0);
		while(GetMessage(&msg, NULL, 0, 0)) {
		TranslateMessage(&msg);
		if(msg.message == WM_TIMER)
			m_pMainChar->RemoveTActionSFX(&m_pMainChar->m_vTGARBAGESFX);
		DispatchMessage(&msg);
  }
						//m_pMainChar->RemoveTActionSFX(vTGARBAGESFX);
						/*m_pMainChar->GetRidingPet()->SetSpeedWhenRiding(1);
...
VOID CALLBACK CTClientGame::MyTimerProc( 
	...
	case SKILL_SUCCESS		:
		{
			...
			if(pTPET)
			{
				SendCS_PETRIDING_REQ(pTRECALL->m_dwID, TRUE);
					...
					!m_pMainChar->IsAlliance(*itTOBJ) )
				{
					...
					if( (*itTOBJ)->m_fZValue < TPICK_DIST )
					{
						D3DXVECTOR2 vDIR(
							...
							(*itTOBJ)->GetPositionZ() - m_pMainChar->GetPositionZ()); // ƒ≥∏Ø≈Õø°º≠ ≈∏∞Ÿ¿ª ∫∏¥¬ Direction. (ƒ≥∏Ø≈Õ∞° ∫∏¥¬ πÊ«‚¿Ã æ∆¥œ¥Ÿ)
						D3DXVec2Normalize( &vDIR, &vDIR);

						int nTargetDIR = m_pMainChar->GetTargetDIR( vDIR.x, vDIR.y); // Direction¿ª ∞¢µµ∑Œ πŸ≤ﬁ.
						nTargetDIR = abs(nTargetDIR-m_pMainChar->m_wDIR); // ƒ≥∏Ø≈Õ∞° ∫∏∞Ì ¿÷¥¬ πÊ«‚∞˙¿« ¬˜∏¶ ±∏«‘.
						nTargetDIR = nTargetDIR > 900 ? 1800-nTargetDIR : nTargetDIR; // 180µµ ≥—¥¬ ∞ÊøÏ, øπ∏¶ µÈ∏È 270µµ∏¶ 90µµ∑Œ πŸ≤ﬁ.

						(*itTOBJ)->m_fPickWeight =
							(TPICK_DIST- (*itTOBJ)->m_fZValue ) / TPICK_DIST * 43.0f
							+ (900.0f - nTargetDIR) / 900.0f * 57.0f; // «»≈∑∞°¡ﬂƒ°, ∞≈∏Æ¥¬ 43%, ∞¢µµ¥¬ 57%, √÷¥Î∞™¿∫ 100¿Ã¥Ÿ.
						...
						if(!((*itTOBJ)->m_vTSKILLTARGET.empty()))
						{
							...
							for(; itST != endST ; ++itST)
								...
								{
									(*itTOBJ)->m_fPickWeight += 100.0f; // ≥™∏¶ ≈∏∞Ÿ¿∏∑Œ ªÔ∞Ì ¿÷¥¬ ¥ÎªÛ¿ª √÷øÏº±¿∏∑Œ ªÔ¥¬¥Ÿ. (¿ß¿« √÷¥Î∞™ 100¿ª ¥ı«ÿ¡÷¥¬ ¿Ã¿Ø)
									break;
								}
						}
...
BOOL CTClientGame::DoActCmd( const CString& strActCmd )
{
	if( m_pMainChar->IsDead() || m_pMainChar->IsDown() || m_pMainChar->IsFall() || 
		m_pMainChar->IsJump() || m_pMainChar->m_bPrivateShop  ||
		!m_bCanUseSkill )
	{
		return FALSE;
	}
...
void CTClientGame::CalcDynamicHelp( DWORD dwTick)
{
	...
	CTDynamicHelpNewDlg* pDLG = static_cast<CTDynamicHelpNewDlg*>(m_vTFRAME[TFRAME_HELPNEW]);
	
	pDLG->ShowComponent(FALSE);
/*
	CTDynamicHelpDlg* pDLGX = static_cast<CTDynamicHelpDlg*>(m_vTFRAME[TGAUGE_FRAME_NOTIFY]);
	
	pDLGX->ShowComponent(FALSE);
	*/
...
void CTClientGame::OnActionChangeCH(CTMiniPopupDlg* pPopupDlg, CTMiniPopupDlg::CTButtonMP* pButton)
{
	CTClientGame* pGame = static_cast<CTClientGame*>(pPopupDlg->GetParent());
	pGame->OnGM_CHANGE_CHANNEL();
...
//    pDetailInfo->CheckGemstones(pClientItem->m_wItemID,bCurGems);
	CRect rc;
	pDetailInfo->GetComponentRect(&rc);
 // ZÌtra zcompilovat pls
	INT x, y;
...
bool CTClientGame::GetSafeMainTarget(CString& outType, unsigned int& outID)
{
	...
	{
		//MessageBox(0,"CHYBA V KOLIZICH","",1);
		return false;
	}
//MessageBox(0,"CHYBA V KOLIZICH","",1);
return false;
	...
	if(!bMonRoaming)
	{
		...
		for( itMON = m_mapSLAVEMON.begin(); itMON != m_mapSLAVEMON.end(); itMON++)
			if( (*itMON).second && (*itMON).second->m_pMON && (*itMON).second->m_pMON->m_bApplyAI )
			{
				...
				if( pMONSTER->m_bFollowType != OT_NONE ) // π´æ∞°∏¶ µ˚∂Û∞°¥¬ ∏ÛΩ∫≈Õ¥¬ ±◊ ¥ÎªÛ∞˙ «ÿ√º!
				{
					CTClientObjBase *pFollow = FindOBJ(
						pMONSTER->m_dwFollowID,
						pMONSTER->m_bFollowType);
...
CString GetFameTitle( BYTE bType, DWORD dwRank )
{
	...
	{
		0,
		TSTR_TOURNAMENT_ENTRY_1, //ø©Ω≈¿« ±‚ªÁ¥‹
		TSTR_TOURNAMENT_ENTRY_2, //∆»ƒÀ«œ¿Œ¿« »≠Ω≈
		TSTR_TOURNAMENT_ENTRY_3, //∑Á∏∂∆ƒ∏£≈©¿« »≠Ω≈
		TSTR_TOURNAMENT_ENTRY_4, //∆Ê¥Ÿ∆Æ∑–¿« »≠Ω≈
		TSTR_TOURNAMENT_ENTRY_5, //∂Û««∏ÆΩ∫¿« »≠Ω≈
		TSTR_TOURNAMENT_ENTRY_6, //∆ƒøÔµÂ∑–¿« »≠Ω≈
		TSTR_TOURNAMENT_ENTRY_7, //∑πƒ´πÃø°¿« »≠Ω≈
		TSTR_TOURNAMENT_ENTRY_8, //ø£µßµÂ∑ŒΩ∫¿« »≠Ω≈
		TSTR_TOURNAMENT_ENTRY_9 //¿Ã∫•∆Æ
	};
...
void CTClientGame::CalcBOWTick( DWORD dwTick)
{
	/*
	if( m_bDoAutoBOW )
	{
		if( m_dwLeftTickAR <= dwTick )
		{
			...
			if( m_pMainChar->IsDead() || m_pMainChar->m_bGhost )
			{
				CTBowRevDlg* pBOWR = static_cast<CTBowRevDlg*>(m_vTFRAME[TFRAME_BOWREV]);
				pBOWR->ShowComponent(FALSE);
				if(m_pMainChar->m_bContryID == 0 || m_pMainChar->m_bAidCountryID == 0)
				{
				SendCS_REVIVAL_REQ(
					276.73f,
					0,
		...
		else
		{
			...
			if( m_dwLeftTickAR/TNOTIFY_PERIOD != dwLEFT/TNOTIFY_PERIOD )
			{
				...
				pt.x -= 165;
				pt.y += 115;
				pBOWR->MoveComponent(pt);
				CString REV;
				REV.Format("%d",m_dwLeftTickAR/TNOTIFY_PERIOD);
				pBOWR->m_pText->m_strText = REV;
				pBOWR->ShowComponent(TRUE);
			}
...
void CTClientGame::CalcLOTTick( DWORD dwTick)
{
	...
	if(m_bStartedLot)
	{
		...
		{
			m_bStartedLot = 0;
            m_pSESSION->SendCS_CHECKLOTEND_REQ(m_pMainChar->m_dwID);
		...
		{
			DWORD dwLEFT = 	m_dwLotT - dwTick;
          //  m_bAuto = 1;
			if( m_dwLotT/10 != dwLEFT/10 ) //10 ok
			{
	      
	            DWORD dwRand = rand() % 50+25;
				CTLotteryDlg* pLot = static_cast<CTLotteryDlg*>(m_vTFRAME[TFRAME_LOTTERY]);
				CString rand;
				rand.Format("%d",dwRand);
				pLot->m_pStart->m_strText = rand;
