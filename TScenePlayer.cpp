...
BYTE CTSpCode::CalcTick( CTClientGame *pTGAME, DWORD dwTick)
{
	...
	case TSCENE_PLAYER_CODE_SHOW_UI				: 
		...
		pTGAME->EnableUI(TFRAME_UIBAR);
		pTGAME->EnableUI(ID_FRAME_MINIMAP);
	    CTUIBarDlg *pUI = (CTUIBarDlg *) pTGAME->GetFrame(TFRAME_UIBAR);
		pUI->bShownFade = pTGAME->m_vTOPTION.m_bHUD;

		
		}
		break;
	case TSCENE_PLAYER_CODE_HIDE_UI				: 
		{
	

		pTGAME->EnableUI(TFRAME_CINEMATIC); 
		pTGAME->DisableUI(TFRAME_UIBAR);
		pTGAME->DisableUI(ID_FRAME_MINIMAP);
	 //I was able to do it in wnd where is DisableUI function but I noob XD
		CTUIBarDlg *pUI = (CTUIBarDlg *) pTGAME->GetFrame(TFRAME_UIBAR);
		pUI->bShownFade = FALSE;
