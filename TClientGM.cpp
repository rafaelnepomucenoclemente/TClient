...
int CTClientGame::OnGM_MAIN_MENU_NEW()
{
	CPoint poska;
...
pHotDlg->EnableSettings();
ToggleUI(TFRAME_HOTSTART);
//	ToggleUI(TFRAME_MAIN_MENU);
	return TERR_NONE;
}

int CTClientGame::OnGM_SYS_MENU()
{
	OnGM_MAIN_MENU_NEW();
	//ToggleUI(TFRAME_MAIN_MENU);
	return TERR_NONE;
...
//ToggleUI(TFRAME_FQUEST);
	ToggleUI(TFRAME_QUEST);
	if(!pTQuestDlg->IsVisible())
	  DisableUI(TFRAME_HOTSTART);
...
int CTClientGame::OnGM_TOGGLE_CHARINFO()
{
	...
	case TINFO_RANK:
		{
			if( m_vTFRAME[ TFRAME_RANKING ]->IsVisible() )
				ToggleUI( TFRAME_RANKING );
			else
			{
...
int CTClientGame::OnGM_TOGGLE_DEFINVEN()
{
	...
	CTHotStartDlg* pHot = static_cast<CTHotStartDlg*>(m_vTFRAME[TFRAME_HOTSTART]);
	if(pHot->IsVisible())
		pHot->ShowComponent(FALSE);
...
int CTClientGame::OnGM_GOTO_THUMB()
{
	...
	m_pMainChar->m_fSpeedFactor = 1.3f * 1.5f;
...
int CTClientGame::OnGM_HOTCABINET()
{
		CPoint poska;
	CTHotStartDlg* pHotDlg = static_cast<CTHotStartDlg*>(m_vTFRAME[TFRAME_HOTSTART]);
			//CTMainUI* pMainUI = static_cast<CTMainUI*>( CTClientGame::GetInstance()->GetFrame( TFRAME_MAIN ) );
...
//pMsgFrame->SetSelect(TFRAME_BLOCKLIST);
...
/*
	m_pMainWnd->MessageBoxOK(
			_T("Pošta je v tomto patchi zablokovaná. Nejspíše to bude opravené v dalším patchi.."),
			TSTR_OK,
...
int CTClientGame::OnGM_ITEMUP_READY()
{
	...
	else
	{
		...
		case IK_GEM			:	
			strMSG = "If the attaching of a your Gemstone fails, all previous Gemstones will be destroyed from the Item you want to Upgrade ! If it succeeds, the Gemstone Level will be increased by 1. Do you want to try to attach the Gemstone ?"; 
			break;
		...
		case IK_HARMOR:
			{
				if(pUpItem == pMatItem)
				{
					strMSG = "You will reset the item transmogrifie. Do you want to continue?"; 	
				}
				else
				{
					strMSG = "You will transmogrifie your item with your selected item. Do you want to continue?"; 
				}
...
int CTClientGame::OnGM_GUILD_HOTTERRITORY()
{
	ToggleUI(TFRAME_COMMUNITY);
	CTCommunityDlg* pDLG = static_cast<CTCommunityDlg*>(m_vTFRAME[TFRAME_COMMUNITY]);
	pDLG->SetSelect(TCOMMUNITY_FRAME_GUILDTERRITORY);
...
int CTClientGame::OnGM_UTILIRYBAR_EX()
{
	...
	CTClientGame::m_vTOPTION.m_bUIBar = !CTClientGame::m_vTOPTION.m_bUIBar;

	CTUIBarDlg* pBar = static_cast<CTUIBarDlg*>(m_vTFRAME[TFRAME_UIBAR]);
	if(CTClientGame::m_vTOPTION.m_bUIBar == FALSE)
...
{
    PickNext(TPICK_COUNT); //pick none cuz of !pTARGET attr
...
{
	PickNext(TPICK_COUNT); 
	CTShopDlg* pDlg =static_cast<CTShopDlg*>(m_vTFRAME[TFRAME_ITEM_SHOP]);
	pDlg->bOpenByUI = TRUE;
...
//		pPet->ApplyGradeEffect(pPet->m_OBJ,0);
	//	pPet->PlayTActionSFX(408);
	if( pPet )
	{
		...
		{
			if( !pDlg->IsRecalling() ) // SetRecalling(), IsRecalling() ÇÔ¼ö´Â CS_PETRECALL_REQÆÐÅ¶ÀÇ ¿¬ÀÌÀº Áßº¹Àü¼ÛÀ» ¸·´Â ¿ªÈ°.
			{
...
int CTClientGame::OnGM_SELECT_TITLE()
{
	if( !m_pSESSION )
		return TERR_NONE;
	...
	wCharTitleID = m_pMainChar->GetCharTitle();

	m_pSESSION->SendCS_CHGTITLE_REQ( wCharTitleID );
...
/*
	...
	*/
	CString pmsg;
	pmsg = "Pokud máte zájem si zakoupit MK, kontaktujte nás na FB nebo Jirkus (facebook.com/jirkus.cz)";
		m_pMainWnd->MessageBoxOK(
			pmsg,
			TSTR_OK,
			GM_CLOSE_MSGBOX, 0);
...
//CPoint poska;
//CTLotteryDlg* pBOWR = static_cast<CTLotteryDlg*>(m_vTFRAME[TFRAME_LOTTERY]);

//pBOWR->GenerateNumbers();





 //      m_pSESSION->SendCS_RESETLOT_REQ(m_pMainChar->m_dwID);
		pTitleDlg->ClearLists();
	//CTDynamicHelpNewDlg* pDyn = static_cast<CTDynamicHelpNewDlg*>(m_vTFRAME[TFRAME_BOWRANK]);
//	pDyn->SetScale((float)m_pMainWnd->m_Device.m_option.m_dwScreenX / 2,1,1);
//	pDyn->MoveComponent(ptx);
//	pDyn->ShowComponent(TRUE);

	ToggleUI(TFRAME_POTIONS);
...
msg = "Do bitvy se mohou pøihlásit pouze hráèi z Valoriánu a Derionu.";
	CTBowRegister* pBOWReg = static_cast<CTBowRegister*>(m_vTFRAME[TFRAME_BOWREGISTER]);

switch(m_pMainChar->m_bContryID)
{

case TCONTRY_D:
	pBOWReg->m_pText->m_strText = "Derion vytvoøil alianci k boji proti Valoriánu. Potøebujeme tvou pomoc! Chceš se zúèastnit bitvy?";
	break;
case TCONTRY_C:
    pBOWReg->m_pText->m_strText = "Valorián vytvoøil alianci k boji proti Derionu. Potøebujeme tvou pomoc! Chceš se zúèastnit bitvy?";
	break;
}

if(m_pMainChar->m_bContryID == 3||m_pMainChar->m_bContryID == 2)
{
switch(m_pMainChar->m_bAidCountryID)
{
case TCONTRY_D:
	pBOWReg->m_pText->m_strText = "Derion vytvoøil alianci k boji proti Valoriánu. Potøebujeme tvou pomoc! Chceš se zúèastnit bitvy?";
	break;
...
if(m_pMainChar->m_bContryID != TCONTRY_D && m_pMainChar->m_bContryID != TCONTRY_C && m_pMainChar->m_bAidCountryID != TCONTRY_D && m_pMainChar->m_bAidCountryID != TCONTRY_C)
{
m_pMainWnd->MessageBoxOK(
		msg,
		TSTR_OK,
	...
	ToggleUI(TFRAME_BOWREGISTER);
	return TERR_NONE;
}
int CTClientGame::OnGM_SENDBOW_REQ()
{
	
	if( !m_pSESSION )
		return TERR_NONE;

	m_pSESSION->SendCS_BOWREG_REQ(m_pMainChar->m_dwID);
	ToggleUI(TFRAME_BOWREGISTER);
	return TERR_NONE;
}
...
int CTClientGame::OnGM_DURATIONREP_NOR()
{
	CTItemRepairDlg* pDlg = static_cast<CTItemRepairDlg*>(m_vTFRAME[TFRAME_ITEM_REPAIR]);
		...
		m_bDurationRepType = RPT_NORMAL; //ÀÏ¹Ý¼ö¸®
...
int CTClientGame::OnGM_INIT_UI()
{
		...
		m_pMainWnd->MessageBoxOK(
			_T("Pokuï chcete resetovat UI, použijte nastavení v Launcheru. EN: If you want Reset your UI, Use Settings on the Launcher."),
			TSTR_OK,
			GM_CLOSE_MSGBOX,
			0 );
	/*
...
BYTE bInvenID = (BYTE) LOWORD(m_dwCmdParam);
	...
	if( m_pSESSION )
		m_pSESSION->SendCS_SADDLE_REQ( bInvenID, bItemID );
...
int CTClientGame::OnGM_GUILD_INVITATION_REGULAR_OK()
{
	...
	if( pDlg->GetMode() == 1 )  // Edit
	{
		if( pDlg->m_pEditArticleTitle->m_strText.IsEmpty() )
			return TERR_GUILD_NOINPUT_ARTICLE_TITLE; // Á¦¸ñÀ» ÀÔ·ÂÇÏ¼¼¿ä.

		if( pDlg->m_pEditLevelMin->m_strText.IsEmpty() )
			return TERR_GUILD_NOINPUT_ARTICLE_MINLEVEL; // ÃÖ¼Ò·¹º§À» ÀÔ·ÂÇÏ¼¼¿ä.

		if( pDlg->m_pEditLevelMax->m_strText.IsEmpty() )
			return TERR_GUILD_NOINPUT_ARTICLE_MAXLEVEL;  // ÃÖ´ë·¹º§À» ÀÔ·ÂÇÏ¼¼¿ä.
...
int CTClientGame::OnGM_RANK_INFO_TOTAL_TAB()
{
	...
	//CTRankingDlg::m_bTabIndex = CTRankingDlg::TRANKINFO_TAB_TOTAL;
	pDlg->ResetData( 0,0 );
...
int CTClientGame::OnGM_RANK_INFO_COUNTRY_TAB()
{
	...
	//CTRankingDlg::m_bTabIndex = CTRankingDlg::TRANKINFO_TAB_COUNTRY;
	pDlg->ResetData( 1,1 );
...
int CTClientGame::OnGM_RANK_INFO_TOURNAMENT_TAB()
{
	...
	//CTRankingDlg::m_bTabIndex = CTRankingDlg::TRANKINFO_TAB_TOURNAMENT;
	pDlg->ResetData( 2,2 );
...
int CTClientGame::OnGM_LOTTERY_START()
{
	CTLotteryDlg* pLot = static_cast<CTLotteryDlg*>(m_vTFRAME[TFRAME_LOTTERY]);
		pLot->m_pStart->m_menu[TNM_LCLICK] = 0;
	m_bStartedLot = TRUE;
	m_dwLotT = 2500;

	return TERR_NONE;
}
int CTClientGame::OnGM_LOTTERY_END()
{
	DisableUI(TFRAME_LOTTERY);
	DisableUI(TFRAME_LOTWIN);
...
int CTClientGame::OnGM_LOT_TRY()
{
//m_pSESSION->SendCS_LOT_RESET();

CTMainUI* pMainUI = static_cast<CTMainUI*>(m_vTFRAME[TFRAME_MAIN]);
pMainUI->m_pLOT->ShowComponent(FALSE);
pMainUI->m_pCashShopButton->SetGauge(0,100);


		if( m_pSESSION)
	{
		m_pSESSION->SendCS_RESETLOT_REQ(
			m_pMainChar->m_dwID );
	}
