...
#define TSTR_CUSTOM_TITLE_INVALID	"Nemùžeš si vybrat tento titul."
#define TSTR_CUSTOM_TITLE_AQUIRE	"Získal jsi titul \"%s\""
#define TSTR_CUSTOM_TITLE_CHANGE	"\"%s\" se zmìnil na \"%s\""
#define TSTR_CUSTOM_TITLE_LOOSE		"Pøišel jsi o titul \"%s\""
...
int CTClientGame::OnCS_CONNECT_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	case CN_SUCCESS			:
		{
		    CPoint poska;
	        CTHotStartDlg* pHotDlg = static_cast<CTHotStartDlg*>(m_vTFRAME[TFRAME_HOTSTART]);
			CPoint pt = CTClientUIBase::m_vBasis[TBASISPOINT_RIGHT_TOP];
			pt.x -= 230;
			pt.y += 55;
			pHotDlg->MoveComponent(pt);
	...
	for( BYTE i=0; i<bCount; i++)
	{
		...
		else
		{
			...
			if( pSESSION )
			{
				...
				pSESSION->m_bLogicalValid = FALSE; // CS_CONNECT_ACK¸¦ ¹Þ±âÀü±îÁø ÀÌ ¼ÒÄÏÀº ¿¬°áÀº À¯È¿ÇØµµ Åë½ÅÇØ¼­´Â ¾ÈµÈ´Ù.
...
int CTClientGame::OnCS_CHARINFO_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	pMain->m_pCashShopButton->SetGauge(0,20000,0);
	pMain->m_pLOT->ShowComponent(FALSE);
	m_vTTEXT[TTEXT_PLAYER_LOT]->m_strText = "0%";
	...
	if(m_vMAP.m_wMapID == 3000 && m_pMainChar->m_dwHP <= 0)
	{       m_bDoAutoBOW = TRUE;
			m_dwLeftTickAR = 25000;		
	}
	m_dwRemainSecondChangeAID *= 1000; // sec to ms
	m_pMainChar->SetEXP( dwEXP );
	m_pMainChar->SetCharTitle( wCharTitleID ); //CUSTOM
	...
	CString bChannel;
	bChannel.Format("%d",m_pMainWnd->m_bChannel);
	pMainUI->m_Channel->m_strText = bChannel;
	...
	//AddKid(m_vTFACEIMG[TFACEIMG_PLAYER]);

	//RemoveKid(m_vTFACEIMG[TFACEIMG_PLAYER]);
...
m_vTTEXT[TTEXT_PLAYER_RUNE]->m_strText = "";
	m_vTTEXT[TTEXT_PLAYER_LUNA]->m_strText = "";
//CTMainUI* pMainUI = static_cast<CTMainUI*>(m_vTFRAME[TFRAME_MAIN]);
pMainUI->m_Povest->m_strText = CTChart::Format( TSTR_HONOR, m_pMainChar->m_dwPvPMonthPoint, m_pMainChar->m_dwPvPTotalPoint );
pMainUI->m_BU->m_strText = CTChart::Format( TSTR_MERIT, m_pMainChar->m_dwPvPUseablePoint );


	if(m_vMAP.m_wMapID != 3000)
{
pMainUI->m_Povest->ShowComponent(TRUE);
pMainUI->m_BU->ShowComponent(TRUE);
pMainUI->m_H1->ShowComponent(TRUE);
pMainUI->m_pLuckyNew->ShowComponent(TRUE);
for(int i=0;i<8;i++)
pMainUI->m_pTMAINBTN[i]->ShowComponent(TRUE);
pMainUI->m_pSettings->ShowComponent(TRUE);
CPoint pos;
pMainUI->m_pSettings->GetComponentPos(&pos);
pos.x -= 82;
pMainUI->m_pChar->MoveComponent(pos);
pMainUI->m_pChar->ShowComponent(TRUE);
pMainUI->m_pLuckyText->ShowComponent(TRUE);
pMainUI->m_Clock->ShowComponent(TRUE);
pMainUI->m_ClockTex->ShowComponent(TRUE);
pMainUI->m_Povest->m_strText = CTChart::Format( TSTR_HONOR, m_pMainChar->m_dwPvPMonthPoint, m_pMainChar->m_dwPvPTotalPoint );
pMainUI->m_BU->m_strText = CTChart::Format( TSTR_MERIT, m_pMainChar->m_dwPvPUseablePoint );
}
else
{
pMainUI->m_Povest->ShowComponent(FALSE);
pMainUI->m_BU->ShowComponent(FALSE);
pMainUI->m_H1->ShowComponent(FALSE);
for(int i=0;i<8;i++)
pMainUI->m_pTMAINBTN[i]->ShowComponent(FALSE);
CPoint pos;
pMainUI->m_pSettings->GetComponentPos(&pos);
pos.x -= 36;
pMainUI->m_pChar->MoveComponent(pos);
...
pMainUI->m_pSettings->ShowComponent(TRUE);
pMainUI->m_Clock->ShowComponent(FALSE);
pMainUI->m_ClockTex->ShowComponent(FALSE);
pMainUI->m_pLuckyNew->ShowComponent(FALSE);
pMainUI->m_pLuckyText->ShowComponent(FALSE);
pMainUI->m_Povest->m_strText = CTChart::Format( TSTR_HONOR, m_pMainChar->m_dwPvPMonthPoint, m_pMainChar->m_dwPvPTotalPoint );
pMainUI->m_BU->m_strText = CTChart::Format( TSTR_MERIT, m_pMainChar->m_dwPvPUseablePoint );
}
...
int CTClientGame::OnCS_UPDATELOT_REQ(CTachyonSession*pSession,CPacket*pPacket)
{
	DWORD dwLotExp;
	DWORD dwNewLotExp;
	(*pPacket)
	...
	pMain->m_pCashShopButton->SetGauge(dwLotExp,dwNewLotExp,0);
	if(dwLotExp == dwNewLotExp)
	{
    pMain->m_pLOT->ShowComponent(TRUE);
	}
	else
	pMain->m_pLOT->ShowComponent(FALSE);
	CString strLot;

	int result = dwLotExp * 100 /dwNewLotExp;
	CString percenta;
	percenta.Format("%d%%",result);
	m_vTTEXT[TTEXT_PLAYER_LOT]->m_strText = percenta;
	return TERR_NONE;
}
int CTClientGame::OnCS_GETLREWARD_REQ(CTachyonSession*pSession,CPacket*pPacket)
{
	DWORD dwGold, dwSilver, dwCron;
	
	(*pPacket)
		>>dwGold
		>>dwSilver
		>>dwCron;
	CTLotteryDlg* pLot = static_cast<CTLotteryDlg*>(m_vTFRAME[TFRAME_LOTTERY]);
	pLot->m_pStart->m_menu[TNM_LCLICK] = GM_LOTTERY_START;
	CString strG, strS, strB;
	strG.Format("%d",dwGold);
	strS.Format("%d",dwSilver);
	strB.Format("%d",dwCron);
	pLot->m_pRune->m_strText = strG;
	pLot->m_pSilver->m_strText = strS;
	pLot->m_pCrons->m_strText = strB;
	pLot->GenerateNumbers();
	pLot->m_pStart->m_strText = "??";
	EnableUI(TFRAME_LOTTERY);
	return TERR_NONE;
}
...
WORD wCharTitleID;
	...
	//	>> dwBOWTime
		...
		>> bCount;
		if( wCharTitleID != 0 )
		pCHAR->SetCharTitle( wCharTitleID );

	if(dwBOWTime != 60000)
	{
	DWORD dwConvertedTick;
	DWORD dwConvertedSecs;
	DWORD dwCovnertedMins;
	dwConvertedTick = dwBOWTime / 1000;
	dwConvertedSecs = dwConvertedTick % 60;
	dwCovnertedMins = dwConvertedSecs/60;
	CString time;
	time.Format("%d:%d",dwCovnertedMins,dwConvertedSecs);
	CPoint poska;
	CTBowAlarm* pHotDlg = static_cast<CTBowAlarm*>(m_vTFRAME[TFRAME_BOWALARM]);
	CPoint pt = CTClientUIBase::m_vBasis[TBASISPOINT_CENTER_BOTTOM];
	pt.x -= 20;
	pt.y -= 275;
	pHotDlg->MoveComponent(pt);
	pHotDlg->m_pText->m_strText = "Bitva svìtù brzy zaène. Registruj se do bitvy a podpoø tak svùj národ! Bitva zaène za :";//CZECH
	}
	
    CTMainUI* pMainUI = static_cast<CTMainUI*>( m_vTFRAME[ TFRAME_MAIN ] );
	CString Medals;
	Medals.Format("%d",m_dwMedals);
	//pMainUI->m_pMedals->m_strText = Medals;
	...
	for( BYTE i=0; i<bCount; i++)
	{
		...
		if(pTMAINTAIN->m_pTSKILL)
		{
			...
			/*if(pTMAINTAIN->m_pTSKILL->m_wSkillID == 1218 && m_pMainChar->IsRide())
			{

			pPET->InsertMaintain( pTMAINTAIN->m_vTSKILLDATA.m_wID, pTMAINTAIN);
			pPET->PlayTActionSFX(
				&pTMAINTAIN->m_vTSFX,
				pTMAINTAIN->m_pTSKILL->m_dwSFX[TSKILLSFX_LOOP]);
...
int CTClientGame::OnCS_ADDMON_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if(bNewMember)
	{
		...
		if(pMON->m_pMON)
		{
			...
			{
								pMON->PlayTActionSFX( //custom play act
					&pMON->m_vTGARBAGESFX,
					1010);
			}
		}
...
int CTClientGame::OnCS_WLLOT_REQ(CTachyonSession* pSession, CPacket* pPacket)
{
	BYTE bType;
	DWORD dwGold;
	DWORD dwSilver;
	...
	(*pPacket)
	    ...
	    num1.Format("%d",pLot->dwNum1);
		num2.Format("%d",pLot->dwNum2);
		num3.Format("%d",pLot->dwNum3);
		num4.Format("%d",pLot->dwNum4);

		if(bType == 1)
	{
		if(bRand == 0)
		pLot->m_pStart->m_strText = num1;
		else if(bRand == 1)
		pLot->m_pStart->m_strText = num2;
		else if(bRand == 2)
		pLot->m_pStart->m_strText = num3;
		else if(bRand == 3)
		pLot->m_pStart->m_strText = num3;
		else
		pLot->m_pStart->m_strText = num1;
		
			//pLot->m_pStart->m_strText = pLot->m_pCisla[bRand]->m_strText; //Generates it with . can be fixed but im too much lazy :D
	}

	CString strG, strS, strB;
         CTLotWinDlg* pLotWin = static_cast<CTLotWinDlg*>(m_vTFRAME[TFRAME_LOTWIN]);
		 strG.Format("%d",dwGold);
		 strS.Format("%d",dwSilver);
		 strB.Format("%d",dwCron);
		 pLotWin->m_pGolds->m_strText = strG;
		 pLotWin->m_pSilvers->m_strText = strS;
		 pLotWin->m_pCrons->m_strText = strB;
        CPoint pos;
		pLot->GetComponentPos(&pos);
		pos.y += 250;
		pLotWin->MoveComponent(pos);
		pLotWin->m_pOK->m_menu[TNM_LCLICK] = GM_LOTTERY_END;
						EnableUI(TFRAME_LOTWIN);

										CString strFMT;
			strFMT.Format( "%d získal %d Zlata %d Støíbra a %d Bronzu z loterie!",
				m_pMainChar->m_strNAME, dwGold, dwSilver, dwCron);
		

		
										//		DisableUI(TFRAME_LOTTERY);
...
m_pMainChar->GetRidingPet()->SetSpeedWhenRiding(2.5f);
//	m_pMainChar->GetRidingPet()->m_fSpeedFactor = dwSpeedFactor;
...
/*int CTClientGame::OnCS_WHIP_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	m_pMainChar->GetRidingPet()->SetSpeedWhenRiding(dwSpeedFactor);
	m_pSESSION->SendCS_PETRIDING_REQ(m_pMainChar->GetRidingPet()->m_dwID, 1);
...
int CTClientGame::OnCS_SKILLUSE_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	DWORD dwAttackID;
	WORD wSkillID;
	WORD wBackSkillID;	// ½ºÅ³ »ç¿ë½Ã ¹Ý°Ý½ºÅ³(¾ÆÀÌÅÛ½ºÅ³)ÀÇ ¾ÆÀÌµð
...
int CTClientGame::OnCS_DEFEND_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if(pTDEFEND)
	{
		...
		if( pTSKILL && (pTDEFEND->m_dwHP || pTSKILL->m_bStaticWhenDie) )
		{
			...
			//	m_pMainChar->m_fSpeedFactor = 2.5f;
			
						pPet->SetSpeedWhenRiding(2.5f);
...
int CTClientGame::OnCS_LEVEL_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if(pCHAR)
	{
		...
		{
			CheckQuestOBJ(); // ·¹º§¾÷ ÇßÀ»¶§ ÇÒ ¼ö ÀÖ´Â Äù½ºÆ® Á¤º¸¸¦ ´Ù½Ã ¹Þ¾Æº»´Ù.
	...
	if( pDIE == m_pMainChar )
	{
		...
		else
		{
			...
			if( FindInven(IT_USE, IK_REVIVAL, bInven, bSlot) )
			{
				...
				{
					if( m_dwWaitingRevivalItem == 0 ) // ºÎÈ° ÁÖ¹®¼­ Àç»ç¿ë½Ã°£ÀÌ 0ÀÌ¸é ¹Ù·Î »ç¿ë
					{
...
int CTClientGame::OnCS_ITEMUPGRADE_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	else if( bResult == ITEMUPGRADE_TRANS_SUCCESS )
	{
		CTClientItem* pItem = pDlg->GetLastClientItem(CTItemUpDlg::TARGET_SLOT);
		if( pItem )
			pItem->SetColor( wColor );
		...
		if (wColor != 0)
		{
			strMSG = "Your item is successful transmogrified."; 
		}
		else
		{
			strMSG = "This item cannot be transmogrified with the second item."; 
		}
	}
	else if( bResult == ITEMUPGRADE_TRANSRES_SUCCESS )
	{
		CTClientItem* pItem = pDlg->GetLastClientItem(CTItemUpDlg::TARGET_SLOT);
		if( pItem )
			pItem->SetColor( wColor );
		...
		strMSG = "Your item transmogrifie is successful cleared."; 
	}
	...
	else if( bResult == ITEMUPGRADE_GEM_SUCCESS )
	{
		...
		strMSG = "The Gemstone has been successfully attached ! The Gemstone Level has been increased by 1."; 
	...
	else
	{
		...
		case ITEMUPGRADE_DOWNGRADE:
			{
				strMSG = CTChart::LoadString( TSTR_ITEMUP_DOWNGRADE ); // ?? ??? ??????! (?? ??? ?? ??? ????? ???? ????)
		...
		case ITEMUPGRADE_GEM_FAIL:
			{
				strMSG = "The attaching of the Gemstone has failed. All Gemstones have been destroyed !"; 
			...
			break;
		case ITEMUPGRADE_GEM_FAIL_BLUE:
			{
				strMSG = "The attaching of the Gemstone has failed. Gemstones have been destroyed except two!"; 
		...
		case ITEMUPGRADE_NOGEM:
			{
				strMSG = "You have already reached the highest Gemstone Level !"; 
			}
			break;

		case ITEMUPGREAD_NOGRADE:
			strMSG = CTChart::LoadString( TSTR_ITEMUP_ERR_CANNOTGRADE ); // ? ???? ??? ? ????.
			break;
...
int CTClientGame::OnCS_PARTYATTR_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if(pPC)
	{
		...
		if( pPC == m_pMainChar )
		{
			((CTMainUI *) m_vTFRAME[TFRAME_MAIN])->m_pPartyIT->EnableComponent( m_pMainChar->m_dwChiefID == m_pMainChar->m_dwID );
		}
	...
	case TNPC_ITEM:
		{
			...
			if(wNpcID == 26012)
				pShopDlg->bOpenByUI = TRUE;
			else
				pShopDlg->bOpenByUI = FALSE;
				...
				if( pTQUEST )
				{
					...
					else if( bQuestSt == THASQUEST_COMPLETE )
					{
						...
						if( !pTMISSION )
						{
							...
							{
								CString strMsg;
								strMsg.Format( "%u Äù½ºÆ®ÀÇ TermÁß¿¡ ¿Ï·áÄù½ºÆ®°¡ ÀÖ´Âµ¥ ±× Äù½ºÆ® ¾ÆÀÌµð¸¦ Ã£À» ¼ö ¾ø´Ù.",
									dwQuestID);
...
int CTClientGame::OnCS_SKILLEND_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if(pOBJ)
	{
		...
		if( pTMAINTAIN != NULL )
		{
			...
			// finder¸¦ Áö¿î´ÙÀ½¿¡ AdjustSDT_STATUS_MOVESPEED¸¦ ½ÇÇà½ÃÅ°´Â ÀÌÀ¯´Â
			// ÀÌ ÇÔ¼ö´Â ÀüÃ¼ MAINTAINÀ» ¼øÈ¸ÇÏ¸é¼­ °¡Àå ³·Àº ÀÌµ¿¼Óµµ¸¦ Ä³¸¯ÅÍÀÇ ¼Óµµ·Î ¼±ÅÃÇÏ´Â ÀÏÀ» ÇÏ´Âµ¥
			// finder´Â ¼­¹ö°¡ Áö¿ì¶ó°í ÇÑ MAINTAINÀÌ±â ¶§¹®¿¡ ¾ê¸¦ Æ÷ÇÔÇØ¼­ ¼øÈ¸ÇÏ¸é ¾ÈµÇ±â ¶§¹®ÀÌ´Ù.
			if( bMoveSpeed )
			{
...
int CTClientGame::OnCS_POSTSEND_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	case POST_SUCCESS:
		{
			...
			if( pDlg->m_bSendByCashItem == 2 ) // Ä³½¬¾ÆÀÌÅÛ¿¡ ÀÇÇØ¼­ ÆíÁöº¸³»±â°¡ ÇàÇØÁø °ÍÀÌ¶ó¸é
			{
	...
	pDlg->ClearUsingByCash(); // Ä³½¬¾ÆÀÌÅÛ¿¡ ÀÇÇØ¼­ ÆíÁöº¸³»±â ÇÒ¶§ »ç¿ëÇÏ´Â Á¤º¸µéÀº ÀÌÁ¦ ÇÊ¿ä¾øÀ¸´Ï ¸®¼Â.
	return TERR_NONE;
}
...
int CTClientGame::OnCS_GETSADDLE_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	(*pPacket)
		...
		>> wItemID;
CTPetManageDlg* pDlg = static_cast<CTPetManageDlg*>(m_vTFRAME[TFRAME_PET_MANAGE]);
pDlg->wSedloID = wItemID;
pDlg->UpdateSaddleTime(SdwTime);
return TERR_NONE;
}
...
int CTClientGame::OnCS_CHARTITLE_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	DWORD dwCharID;
	WORD wCharTitleID;
...
int CTClientGame::OnCS_CHGTITLE_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	BOOL bResult;
	...
	if( !bResult )
	{
		m_pMainChar->ResetCharTitle();

		CString strTYPE;
		strTYPE = CTChart::LoadString( TSTR_INFO_TITLE);
	//	m_pChatFrame->ChatSysMSG( strTYPE, m_pMainChar->GetName(), TSTR_CUSTOM_TITLE_INVALID, TCOLOR_ERROR, TCHAT_FLAG_INFO);
	}

	return TERR_NONE;
}

int CTClientGame::OnCS_TITLEUPDATE_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	BYTE bType;
	WORD wTitleID;
	WORD wSecondTitleID;
	...
	WORD wCharTitleID;
	wCharTitleID = pTCHAR->m_wCharTitleID;

	CString strMsg;

	LPTTITLE pTitle = CTChart::FindTTITLETEMP(wTitleID);
	LPTTITLE pCharTitle = CTChart::FindTTITLETEMP(wCharTitleID);
	LPTTITLE pSecondTitle = CTChart::FindTTITLETEMP(wSecondTitleID);

	// bType - 0 => aquire, 1 => delete, 2 => change

	if( bType == 2 )
	{
		strMsg.Format( TSTR_CUSTOM_TITLE_CHANGE, pTitle->m_strTitle, pSecondTitle->m_strTitle);

		if( pSecondTitle->m_wCategory == pCharTitle->m_wCategory )
		{
			if( pSecondTitle->m_bInCategoryCount != pCharTitle->m_bInCategoryCount )
			{
				pTCHAR->ResetCharTitle();
				pTCHAR->SetCharTitle( pSecondTitle->m_wTitleID );
			}
		}
	}
	else if( bType == 1 )
	{
		strMsg.Format( TSTR_CUSTOM_TITLE_LOOSE, pTitle->m_strTitle);

		if( pTCHAR->GetCharTitle() == pTitle->m_wTitleID )
			pTCHAR->ResetCharTitle();
	}
	else if( bType == 0 )

strMsg.Format(TSTR_CUSTOM_TITLE_AQUIRE, pTitle->m_strTitle);

	m_pSESSION->SendCS_CHGTITLE_REQ( pTCHAR->GetCharTitle() );
...
int CTClientGame::OnCS_TITLELIST_ACK(CTachyonSession *pSession, CPacket *pPacket)
{
	...
	CTTitleDlg *pDlg = static_cast<CTTitleDlg*>(m_vTFRAME[TFRAME_TITLE]);
	CTClientChar* pTCHAR = CTClientGame::GetInstance()->GetMainChar();

	pDlg->m_pList2->RemoveAll();
	...
	for(BYTE i = 0; i < bCount; ++i)
	{
		...
		int nLine = pDlg->m_pList2->AddString(CString(), 1);

		if(pTitle->m_wTitleID == 0 && !pTCHAR->m_bHasCharTitle)
			pDlg->m_pList2->SetCurSelItem(0);

		if( pTCHAR->m_bHasCharTitle )
		{
			if(pTitle->m_wTitleID == pTCHAR->GetCharTitle())
			{
				pDlg->m_pList2->SetCurSelItem(nLine);
				bValid = TRUE;
			}
		}

		pDlg->m_pList2->SetItemString(nLine, 0, pTitle->m_strTitle);
		pDlg->m_pList2->SetItemData(nLine, 0, pTitle->m_wTitleID);
		pDlg->m_pList2->SetItemData(nLine, 1, pTitle->m_wTitleID);
	}

	if( !bValid )
	{
		pTCHAR->ResetCharTitle();
		m_pSESSION->SendCS_CHGTITLE_REQ(0);
	}
...
int CTClientGame::OnCS_GUILDLOCALLIST_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	for(BYTE i=0; i<wCount; ++i)
	{
		...
		(*pPacket)
			...
			>> info.m_bDefCount //Áö¿øÀÚ ¼ö
			>> info.m_wAtkGuildPoint
			>> info.m_wAtkCountryPoint
			>> info.m_bAtkCount //Áö¿øÀÚ ¼ö
...
int CTClientGame::OnCS_GUILDTACTICSLIST_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	for(BYTE i=0; i<dwCount; ++i)
	{
		...
		(*pPacket)
			...
			>> member.m_dlMoney
			>> member.m_dlEndTime
			>> member.m_dwGotPoint //°è¾à±â°£µ¿¾È ¾òÀº °øÀû Æ÷ÀÎÆ®
			>> member.m_dwRegion
			>> member.m_wCastle
			>> member.m_bCamp;
...
int CTClientGame::OnCS_GUILDTACTICSANSWER_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	CString strMSG;
	BYTE bResult;
	DWORD dwNewTacticsMemberID; //¿ëº´±æµå¾ÆÀÌµð
	...
	if( bResult == GUILD_SUCCESS )
	{
		if( dwNewTacticsMemberID == m_pMainChar->m_dwID ) // ¿ëº´ ´ë»óÀÚ
		{
			EnableGuildUI( TRUE );
		}
		else // ¿ëº´ÃÊ´ë±æµåÀå
	...
	{
		if( dwNewTacticsMemberID == m_pMainChar->m_dwID && m_vMAP.m_wMapID == 3000) // ¿ëº´ ´ë»óÀÚ
		{
		}
		else // ¿ëº´ÃÊ´ë±æµåÀå
		{
...
int CTClientGame::OnCS_PARTYMEMBERRECALL_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if( bType == 0 )
	{
		...
		case IU_NOTFOUND		: //¾ÆÀÌÅÆ¾ø´øÁö Àß¸ø½è´øÁö
			{
...
{
	BYTE bResult;
	DWORD dwCharID;		// ¼Ò¿ï¸ÞÀÌÆ®¾ÆÀÌµð
	CString strName;	// ¼Ò¿ï¸ÞÀÌÆ®ÀÌ¸§
	DWORD dwRegion;
	...
	case SOULMATE_SILENCE	:	//¿µÈ¥ÀÇÄ§¹¬±â
		return TERR_SOULMATE_SILENCE;
	case SOULMATE_NEEDMONEY :	//µ·ºÎÁ·
...
int CTClientGame::OnCS_ENTERSKYGARDEN_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	CTSkygardenMap* pSkygardenMapUI = static_cast< CTSkygardenMap* >( m_vTFRAME[TFRAME_AVSTATUS] );
...
int CTClientGame::OnCS_ENTERBOW_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	if( !m_pMainChar )
		return TERR_NONE;
	...
	m_pMainChar->SetCastleID( wBoWID );


CTBowStatus* pBOWStat = static_cast<CTBowStatus*>(m_vTFRAME[TFRAME_BOWSTATUS]);
CString bValo;
CString bDero;
bValo.Format("%d",bValoP);
bDero.Format("%d",bDeroP);
pBOWStat->m_bValoP->m_strText = bValo;
pBOWStat->m_bDeroP->m_strText = bDero;
	return TERR_NONE;
}
...
//AddKid(m_vTFACEIMG[TFACEIMG_PLAYER]);
	...
	else
	{
		...
		case CCB_TIME:
			{
				...
				if( dwDay != 0 )
				{
					strFMT = CTChart::Format(TSTR_NATIONALITY_WAIT_DAY_HOUR, //Áö¿ø±¹ º¯°æÀº [%dÀÏ %d½Ã°£] ÀÌÈÄ¿¡ °¡´ÉÇÕ´Ï´Ù.
						dwDay,
						dwHour );
				}
				else if( dwHour != 0 )
				{
					strFMT = CTChart::Format(TSTR_NATIONALITY_WAIT_HOUR_MIN, //Áö¿ø±¹ º¯°æÀº [%d½Ã°£ %dºÐ] ÀÌÈÄ¿¡ °¡´ÉÇÕ´Ï´Ù.
						dwHour,
						dwMin );
				}
				else if( dwMin != 0 )
				{
					strFMT = CTChart::Format(TSTR_NATIONALITY_WAIT_MIN_SEC, //Áö¿ø±¹ º¯°æÀº [%dºÐ %dÃÊ] ÀÌÈÄ¿¡ °¡´ÉÇÕ´Ï´Ù.
						dwMin,
						dwSec );
				}
				else
		...
		case CCB_PARTY://ÆÄÆ¼±¸¼ºÁß¿¡´Â ÇÒ¼ö¾øÀ½
			{
				CString strFMT;
		...
		case CCB_TACTICS://¿ëº´Áö¿ø»óÅÂ¿¡¼­´Â ÇÒ¼ö¾øÀ½
			{
				CString strFMT;
		...
		case CCB_GUILD://±æµå°¡ÀÔÁß¿¡´Â ÇÒ ¼ö ¾øÀ½
			{
				CString strFMT;
...
int CTClientGame::OnCS_STOPTHECLOCK_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	{
		if( bItem == INVALID_SLOT ) // °¡¹æÀÇ ±âÇÑ¿¬ÀåÀÌ´Ù.
		{
			CTClientInven* pTINVEN = m_pMainChar->FindTInven(bInvenID);
			pTINVEN->m_dEndTime = dEndTime;
		}
		else // ¾ÆÀÌÅÛÀÇ ±âÇÑ¿¬ÀåÀÌ´Ù.
		{
			CTClientInven* pTINVEN = m_pMainChar->FindTInven(bInvenID);
...
int CTClientGame::OnCS_SYSTEMMSG_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	case SM_TOURNAMENT_STEP:
		{
			...
			if( bGroup == 0 )
			{
				...
				{
					// ¸ðµç ±×·ìÀÇ CurrentStepÀ» bCurrentStepÀ¸·Î ¸ÂÃá´Ù.
...
#ifdef ADD_MISSION
	case SM_MISSION_NORMAL:		//%d (Â¦¼ö: È¦¼ö) ¹Ì¼ÇÁö¿ªÀÇ ÀüÀïÀÌ Á¾·áµÇ¾ú½À´Ï´Ù.
		{
			WORD wValue;
			(*pPacket)
				>> wValue;
	...
	case SM_MISSION_START:		//%d (Â¦¼ö: È¦¼ö) ¹Ì¼ÇÁö¿ªÀÇ ÀüÀïÀÌ ½ÃÀÛµÇ¾ú½À´Ï´Ù.
		{
			WORD wValue;
			(*pPacket)
				>> wValue;
	...
	case SM_MISSION_BOSSDIE:		//%s ¹Ì¼ÇÁö¿ªÀ» %d ±¹°¡°¡ Á¡·ÉÇß½À´Ï´Ù.
		{
	...
	case SM_MISSION_START_ALARM: //%d (Â¦.È¦) ¹Ì¼ÇÀüÀïÀÌ %d(ºÐ/ÃÊ)ÈÄ ½ÃÀÛµË´Ï´Ù.
		{
			WORD wValue;
			DWORD dwTIME;
	...
	case SM_MISSION_END_ALARM:	//%d (Â¦.È¦) ¹Ì¼ÇÀüÀïÀÌ %d(ºÐ/ÃÊ)ÈÄ Á¾·áµË´Ï´Ù.
		{
			WORD wValue;
			DWORD dwTIME;
	...
	case SM_MISSION_PEACE:		//%d (Â¦¼ö: È¦¼ö) ¹Ì¼ÇÁö¿ªÀÇ ÀüÀïÀÌ Á¾·áµÇ¾ú½À´Ï´Ù.
		{
			WORD wValue;
			DWORD dwSecond;
	...
	case SM_MISSION_TIMEOUT:		//%s ¹Ì¼ÇÁö¿ªÀ» Á¡·ÉÇÑ ±¹°¡°¡ ¾ø¾î ¸ó½ºÅÍ Áö¿ªÀ¸·Î ÀüÈ¯µÇ¾ú½À´Ï´Ù. 
		{
			CString strMissionName;
			WORD wCountry;
			(*pPacket)
...
case SM_SKYGARDEN_NORMAL:	//Ãµ°øÀü Áö¿ªÀÌ ÀÏ¹Ý»óÅÂ·Î ÀüÈ¯µÇ¾ú½À´Ï´Ù.
		{
			strMSG = CTChart::Format(TSTR_SKYGARDEN_NORMAL);
		}
		break;

	case SM_SKYGARDEN_START:		//Ãµ°øÀüÀÌ ½ÃÀÛµÇ¾ú½À´Ï´Ù.
		{
			strMSG = CTChart::Format(TSTR_SKYGARDEN_BT_BEGIN);
		}
		break;

	case SM_SKYGARDEN_START_ALARM:	//DWORD Ãµ°øÀüÀÌ %d(ºÐ/ÃÊ)ÈÄ ½ÃÀÛµË´Ï´Ù.
		{
			DWORD dwTIME;
			(*pPacket)
				>> dwTIME;
	...
	case SM_SKYGARDEN_END_ALARM:	//DWORD Ãµ°øÀüÀÌ %d(ºÐ/ÃÊ)ÈÄ Á¾·áµË´Ï´Ù.
		{
			DWORD dwTIME;
			(*pPacket)
				>> dwTIME;
	...
	case SM_SKYGARDEN_PEACE:		//Ãµ°øÀüÀÌ Á¾·áµÇ¾î ÆòÈ­»óÅÂ·Î ÀüÈ¯µÇ¾ú½À´Ï´Ù. ÀÌ Áö¿ª¿¡¼­´Â ÀüÅõ¸¦ ÇÒ ¼ö ¾ø½À´Ï´Ù.
		{
			strMSG = CTChart::Format(TSTR_SKYGARDEN_PEACE);
		}
		break;

	case SM_SKYGARDEN_END:			//Ãµ°øÀü¿¡¼­ %d ±¹°¡°¡ ½Â¸®ÇÏ¿´½À´Ï´Ù.
		{
			CString strMissionName;
			WORD wCountry;
			DWORD dwMapID;
			...
			{
				CTSkygardenEnd* pSkygardenEnd = static_cast< CTSkygardenEnd* >( m_vTFRAME[TFRAME_AVWIN] );
		...
		case SM_BOW_ALARM:
		{
			...
			CString string;
			CTBowAlarm* pHotDlg = static_cast<CTBowAlarm*>(m_vTFRAME[TFRAME_BOWALARM]);
			if(dwTIME > 0 && m_vMAP.m_wMapID != 3000)
			{
			//snoop doggg fk you you are watching this source, so you stole my source fuck you      Written 3.7 at 23:45 (11:45PM) by jiøí
			string.Format("%d:%d",dwMins,dwSecs);
			CPoint poska;
			CPoint pt = CTClientUIBase::m_vBasis[TBASISPOINT_CENTER_BOTTOM];
			pt.x -= 20;
			pt.y -= 275;
			pHotDlg->MoveComponent(pt);
			pHotDlg->m_pText->m_strText = "Bitva svìtù brzy zaène! Zaregistruj se a podpoø tak svùj národ. Bitva zaène za :";
			pHotDlg->m_pTime->m_strText = string;
			pHotDlg->ShowComponent(TRUE);
			pHotDlg->EnableComponent(TRUE);
			}


			if(m_vMAP.m_wMapID == 3000)
			{
			pHotDlg->ShowComponent(FALSE);
			pHotDlg->EnableComponent(FALSE);
			}
			if(dwTIME <= 0)
			{
			pHotDlg->ShowComponent(FALSE);
		...
		case SM_BOW_ENDALARM:
		{
			...
			for(int i=0;i<8;i++)
			pMainUI->m_pTMAINBTN[i]->ShowComponent(FALSE);
			pMainUI->m_pMAIL->ShowComponent(FALSE);
			pMainUI->m_pMESSENGER->ShowComponent(FALSE);
			pMainUI->m_pSettings->ShowComponent(TRUE);
			CPoint pos;
			pMainUI->m_pSettings->GetComponentPos(&pos);
			pos.x -= 36;
			...
			pBOWStat->ShowComponent(TRUE);
			pBOWStat->EnableComponent(TRUE);
			pBOWStat->SetTime(dwMins,dwSecs);

			pBOWStat->MoveComponent(pt);
			...
			if(m_vMAP.m_wMapID != 3000)
			{
			pBOWStat->ShowComponent(FALSE);
			pBOWStat->EnableComponent(FALSE);
			pMainUI->m_Povest->ShowComponent(TRUE);
			pMainUI->m_pLuckyNew->ShowComponent(TRUE);
			pMainUI->m_pLuckyText->ShowComponent(TRUE);
			pMainUI->m_H1->ShowComponent(TRUE);
			...
			pMainUI->m_pMAIL->ShowComponent(TRUE);
			pMainUI->m_pMESSENGER->ShowComponent(TRUE);
			pMainUI->m_pSettings->ShowComponent(TRUE);
			CPoint pos;
			pMainUI->m_pSettings->GetComponentPos(&pos);
			pos.x -= 82;
...
int CTClientGame::OnCS_TITLE_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	DWORD dwCharID;
	...
	CTClientChar* pCHAR = FindPC(dwCharID);
	if( pCHAR )
		pCHAR->m_strCharTitle = strCharTitle;
...
int CTClientGame::OnCS_PVPPOINT_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	CTGaugePannel* pTGaugePannel = (CTGaugePannel*) m_vTFRAME[TFRAME_GAUGE];
	/*
	m_vTTEXT[TTEXT_PLAYER_RUNE]->m_strText = CTChart::Format( TSTR_HONOR, m_pMainChar->m_dwPvPMonthPoint, m_pMainChar->m_dwPvPTotalPoint );
	m_vTTEXT[TTEXT_PLAYER_LUNA]->m_strText = CTChart::Format( TSTR_MERIT, m_pMainChar->m_dwPvPUseablePoint );
	*/
m_vTTEXT[TTEXT_PLAYER_RUNE]->m_strText = "";
	m_vTTEXT[TTEXT_PLAYER_LUNA]->m_strText = "";
//	CTMainUI* pMainUI = static_cast<CTMainUI*>(m_vTFRAME[TFRAME_MAIN]);
...
int CTClientGame::OnCS_PVPRECORD_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	DWORD dwRankOrder;
	BYTE bRankPercent;
	DWORD dwMonthRankOrder = 0;			// ¼øÀ§
	BYTE bMonthRankPercent = 0;			// ÆÛ¼¾Æ®
	WORD wMonthWin = 0;					// ½Â·ü
	WORD wMonthLose = 0;
	...
	if ( !bType )
	{
		(*pPacket)
			>> dwMonthRankOrder			// ¼øÀ§
			>> bMonthRankPercent			// ÆÛ¼¾Æ®
			>> wMonthWin					// ½Â·ü
			>> wMonthLose;
	}
	pDLG->ResetData(
		bType ? TINFO_PVP : TINFO_WAR, // 0:ÀüÀï 1:´ë°á 
		dwRankOrder,
		bRankPercent,
		dwKillCount,
		dwDieCount,
...
#ifdef TEST_MODE
	m_dwLeftTickKICKOUTMAP = 1000*60; //1ºÐ
#else
	m_dwLeftTickKICKOUTMAP = 3600000; //1½Ã°£
#endif
...
int CTClientGame::OnCS_GUILDTACTICSREPLY_ACK( CTachyonSession *pSession, CPacket *pPacket)
{
	...
	if( dwTarget == m_pMainChar->m_dwID ) // ¿ëº´°¡ÀÔ ´ë»óÀÚ
	{
		if( bResult == GUILD_SUCCESS )
			EnableGuildUI(TRUE);
	}
...
{
	CTRankingDlg* pDlg = static_cast<CTRankingDlg*>( m_vTFRAME[TFRAME_RANKING] );
...
int CTClientGame::OnCS_UPDATEFAMERANKLIST_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	...
	for( BYTE b=0 ; b < bTypeCount ; ++b )
	{
		...
		if(bUpdateUI) // ¸í¼ºUI Á¤º¸µµ °»½ÅÇÏÀÚ.
			pDlg->ClearRank();
...
//	bMins.Format("%d",bMin);
    ...
    if(bHour == 1||bHour == 2||bHour == 3||bHour == 4||bHour == 5||bHour == 6||bHour == 7||bHour == 8||bHour == 9||bHour == 10||bHour == 11||bHour == 12)
	{
					if(bMin == 0)
		{
			 ...
			 time.Format("AM %d : 00",bHour); 
		}
		else if(bMin == 1)
		{
			 time.Format("AM %d : 01",bHour); 
		}
				else if(bMin == 2)
		{
			 time.Format("AM %d : 02",bHour); 
		}
				else if(bMin == 3)
		{
			
			 time.Format("AM %d : 03",bHour); 
		}
				else if(bMin == 4)
		{
			 time.Format("AM %d : 04",bHour); 
		}
				else if(bMin == 5)
		{
			 time.Format("AM %d : 05",bHour); 
		}
				else if(bMin == 6)
		{
			 time.Format("AM %d : 06",bHour); 
		}
				else if(bMin == 7)
		{
			 time.Format("AM %d : 07",bHour); 
		}
				else if(bMin == 8)
		{
			 time.Format("AM %d : 08",bHour); 
		}
				else if(bMin == 9)
		{
			 time.Format("AM %d : 09",bHour); 
		} 
		
				else if(bMin > 9)
		    time.Format("AM %d : %d",bHour, bMin); 
	}
	...
	else
	{
				if(bMin == 0)
		{
			 ...
			 time.Format("PM %d : 00",bHour); 
		}
		else if(bMin == 1)
		{
			 time.Format("PM %d : 01",bHour); 
		}
				else if(bMin == 2)
		{
			 time.Format("PM %d : 02",bHour); 
		}
				else if(bMin == 3)
		{
			 time.Format("PM %d : 03",bHour); 
		}
				else if(bMin == 4)
		{
			 time.Format("PM %d : 04",bHour); 
		}
				else if(bMin == 5)
		{
			 time.Format("PM %d : 05",bHour); 
		}
				else if(bMin == 6)
		{
			 time.Format("PM %d : 06",bHour); 
		}
				else if(bMin == 7)
		{
			 time.Format("PM %d : 07",bHour); 
		}
				else if(bMin == 8)
		{
			 time.Format("PM %d : 08",bHour); 
		}
				else if(bMin == 9)
		{
			 time.Format("PM %d : 09",bHour); 
		} 
		
				else if(bMin > 9)
		    time.Format("PM %d : %d",bHour, bMin); 
	}
...
int CTClientGame::OnCS_KILLSTATUE_REQ( CTachyonSession *pSession, CPacket *pPacket )
{
	CTChatCommander *pChatCommander = CTChatCommander::GetInstance();
	BYTE bValoP;
	BYTE bDeroP;
	...
	CString vText,dText;
	vText = "Derion znièil Valoriánskou boží sochu!";
	dText = "Valorián znièil Derionskou boží sochu!";
	CTMainUI *pMAIN = static_cast<CTMainUI*>(m_vTFRAME[ TFRAME_MAIN ]);
	if(m_vMAP.m_wMapID == 3000)
	{
	if(bNation == 0)
	pMAIN->ResetChapterMSG(vText); //CZECHC
	else
	pMAIN->ResetChapterMSG(dText);
	}
...
int CTClientGame::OnCS_BOWANNOUNCE_ACK(CTachyonSession*pSession, CPacket*pPacket)
{
	BYTE bType;
	CString strAnnounce;
    ...
    (*pPacket)
		...
		CTMainUI *pMAIN = static_cast<CTMainUI*>(m_vTFRAME[ TFRAME_MAIN ]);
		pMAIN->ResetRegionMSG(strAnnounce); //CZECHC


	return TERR_NONE;
}

int CTClientGame::OnCS_BOWTICK_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	CTChatCommander *pChatCommander = CTChatCommander::GetInstance();
	DWORD dwTick;
	...
	for( BYTE i=0 ; i < bEntryCount ; ++i )
	{
		...
		for( int r=0 ; r < pMatch->m_bRewardCount ; ++r )
		{
			...
			(*pPacket)
				...
				>> pReward->m_bChartType // 0:Äù½ºÆ® 1:¾ÆÀÌÅÛÂ÷Æ®
				>> pReward->m_wItemID
				>> pReward->m_bCount;
	...
	for( int i=0 ; i < bEntryCount ; ++i )
	{
		...
		for( int j=0 ; j < bJoinCount ; ++j )
		{
			...
			(*pPacket)
				...
				>> pPlayer->m_bQfWin // 8°­ ½ÂÆÐ
				>> pPlayer->m_bSfWin // 4°­ ½ÂÆÐ
				>> pPlayer->m_bFWin; // °á½Â ½ÂÆÐ
...
int CTClientGame::OnCS_TOURNAMENTEVENTLIST_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	...
	for( int i=0 ; i < bEntryCount ; ++i )
	{
		...
		(*pPacket)
			>> pMatch->m_bEntryID // Á¾¸ñ
			>> pMatch->m_strEntry //Á¾¸ñÀÌ¸§
			>> pMatch->m_bType // ±¸ºÐ
			>> pMatch->m_strChoice
			>> pMatch->m_bCountry
			>> pMatch->m_fRate //ºñÀ²
			>> pMatch->m_dwAmount; //´çÃ»±Ý
...
int CTClientGame::OnCS_TOURNAMENTENTER_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	...
	// SlotID°¡ ÀÛÀº ³ðÀÌ LeftTeam, SlotID°¡ Å« ³ðÀÌ RightTeam
	...
	if( m_bTournamentWatching )
	{
		...
		//Åä³Ê¸ÕÆ® %d°æ±âÀå¿¡ ÀÔÀåÇÏ¼Ì½À´Ï´Ù. %s´ÔÀº ÀÌ °æ±âÀåÀÇ °ü¶÷ÀÚÀÔ´Ï´Ù.",
	}
	else if( m_bTournamentBattle )
	{
		DisableUI( TFRAME_TOURNAMENT_GAGE );
		// Åä³Ê¸ÕÆ® %d°æ±âÀå¿¡ ÀÔÀåÇÏ¼Ì½À´Ï´Ù. %s´ÔÀº ÀÌ °æ±âÀåÀÇ ¼±¼öÀÔ´Ï´Ù.
	}
...
int CTClientGame::OnCS_TOURNAMENTRESULT_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	...
	// ½ÂÀÚ¿Í ÆÐÀÚ°¡ ¾î´À ÆÀ¿¡ ÀÖ´ÂÁö ¾Ë¾Æ³»±â.
...
int CTClientGame::OnCS_ARENA_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	...
	case ARENA_CHIEF://ÆÄÆ¼Àå¸¸ ½ÅÃ»°¡´É
		m_pMainWnd->MessageBoxOK(
			...
			FALSE );
		break;
	case ARENA_MAXPARTY://ÀÔÀå ÃÖ´ëÀÎ¿ø ÃÊ°ú
		m_pMainWnd->MessageBoxOK(
			...
			FALSE );
		break;
	case ARENA_MINPARTY://ÀÔÀå ÃÖ¼ÒÀÎ¿ø ¹Ì´Þ
		m_pMainWnd->MessageBoxOK(
...
int CTClientGame::OnCS_ASSISTANT_ACK( CTachyonSession *pSession, CPacket *pPacket )
{
	...
	CString strMSG;
	strMSG.Format( "·¹º§%d %s´ÔÀÌ ³Ê·Î Á¤Çß´Ù°í ÇÑ´Ù. ÇÒ·¡?",
		bLevel,
		m_strAssistantInvitor );
