...
BYTE ITDetailInfo::GetGem()
{
	if(pTITEM)
		return pItem->GetGemy();
	else
		return FALSE;
}

BOOL ITDetailInfo::IfGem()
{
	if(pTITEM && pTITEM->m_bCanGrade == 1)
		return TRUE;
	else
		return FALSE;
...
void CTItemDetInfo::_ConvertAndAddLine(const CString& strSource, DWORD dwColor)
{
	...
	//const BYTE btest = pItem->GetGem();
	//CString strGemLevel;
	//strGemLevel.Format("%d", pItem->GetGem());
	BOOL bAddLine = FALSE;
...
void CTOptionItemDetInfo::_ConvertAndSetTitle(const CString& strSource, DWORD dwColor)
{
	...
	CString strGradeColor=_T(") ");
	switch(m_pItemInst->GetGradeEffect())
	{
		case IE_SEA:
			strGradeColor.Append(_T("(Voda"));
			strGrade.Append(strGradeColor);
			break;

		case IE_FIRE:
			strGradeColor.Append(_T("(Láva"));
			strGrade.Append(strGradeColor);
			break;

		case IE_LIGHTING:
			strGradeColor.Append(_T("(Blesk"));
			strGrade.Append(strGradeColor);
			break;

		case IE_ICE:
			strGradeColor.Append(_T("(Led"));
			strGrade.Append(strGradeColor);
			break;

		case IE_BLACK:
			strGradeColor.Append(_T("(Temnota"));
			strGrade.Append(strGradeColor);
			break;

		case IE_NEW1:
			strGradeColor.Append(_T("(Ametist"));
			strGrade.Append(strGradeColor);
			break;

		case IE_NEW2:
			strGradeColor.Append(_T("(Bouøkovì modrá"));
			strGrade.Append(strGradeColor);
			break;

		case IE_NEW3:
			strGradeColor.Append(_T("(Pøírodní zelená"));
			strGrade.Append(strGradeColor);
			break;

		case IE_NEW4:
			strGradeColor.Append(_T("(Magma"));
			strGrade.Append(strGradeColor);
			break;
		default :
			strGradeColor.Append(_T("(Nový Efekt"));
			strGrade.Append(strGradeColor);
			break;
	
		//default:
		//	CString s;
		//	s.Format(_T("%d"), m_pItemInst->GetGradeEffect());
		//	strGradeColor.Append(s);
		//	break;
	}
...
CString CTOptionItemDetInfo::_GetAttrString(TINFOTOKEN_ITEM eToken)
{
	...
	case TINFOTOKEN_ITEM_ATTRLEVEL:
		{
			if( 0 == m_pItemInst->GetViewAttrLevel() )
				return _T("");
		if(m_pItemInst->GetGemy()> 0)
		{
			
			/*if(m_pItemInst->GetGem() % 2 == 1)
			{
				if((m_pItemInst->GetViewAttrLevel() - m_pItemInst->GetGem() + 1) < 34)
				{
				strResult = CTChart::Format( TSTR_FMT_NUMBER, m_pItemInst->GetViewAttrLevel() - m_pItemInst->GetGem() + 2 );
				}
				else
				{
				strResult = CTChart::Format( TSTR_FMT_NUMBER, m_pItemInst->GetViewAttrLevel() - m_pItemInst->GetGem() + 1 );
				}
			}
			else
			{
				if((m_pItemInst->GetViewAttrLevel() - m_pItemInst->GetGem()) < 34)
				{
				strResult = CTChart::Format( TSTR_FMT_NUMBER, m_pItemInst->GetViewAttrLevel() - m_pItemInst->GetGem() + 1 );
				}
				else
				...
				strResult = CTChart::Format( TSTR_FMT_NUMBER, (m_pItemInst->GetViewAttrLevel() - m_pItemInst->GetGemy()));
	...
	/*case TINFOTOKEN_ITEM_GEMSTONELEVEL:
		{
			if( 0 == m_pItemInst->GetGem() )
				return _T(" ");

			strResult = "+" + CTChart::Format( TSTR_FMT_NUMBER, m_pItemInst->GetGem());
			break;
		}*/
...
void CTPlayerDetInfo::Build()
{
	...
	m_wImageID = CTClientGame::GetFACE(
		m_bRace,
		m_bSex,
		m_bFace,
		m_bHair );
	...
	if( m_bSenior )
	{
		_AddLine( "»ó°ü", TDEFAULT_TEXT_COLOR);
	}

	CString strMSG;
	strMSG.Format( "Å¬·¡½º : %s", CTChart::LoadString( (TSTRING) CTClientGame::m_vTCLASSSTR[ m_bClass ] ) );
	_AddLine( strMSG, TDEFAULT_TEXT_COLOR);

	strMSG.Format( "·¹º§ : %d", m_bLevel );
	_AddLine( strMSG, TDEFAULT_TEXT_COLOR);
