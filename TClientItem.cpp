...
LPTITEMGRADEVISUAL CTClientItem::GetGradeVISUAL()
{
	...
	if( m_bGrade )
	{
		...
		{
			wGradeSkin = 0; //kein schimmern das andere is richtig
		}
		else if( 6 <= m_bGrade && m_bGrade <= 10 )
		{
			wGradeSkin = 1; //glitzern1
		}
		else if( 11 <= m_bGrade && m_bGrade <= 15 )
		{
			wGradeSkin = 2; //glitzen2
		}
		else if( 16 == m_bGrade )
		{
			wGradeSkin = 3; //gold 
			//insgesamt farben von 0-12 - ah ok, also sind die resourcen da
			//achja kurze frage, kannst du ein item erstellen mit einer von den neuen fareb, wenn ja wie wird das angezeigt - also ich kann die farbe auf 6 setzen in der db, aber es passiert nix bzw ,k eianles o klappts weil
		}
		...
		{ //ab +17 alle andren
			...
			case IE_NEW3:
				...
				break;
			case IE_NEW4:
				wGradeSkin = 12; //so try 
				break;
			case IE_NEW5:
				wGradeSkin = 14; //so try 
				break;
			case IE_NEW6:
				wGradeSkin = 15; //so try 
				break;
			case IE_NEW7:
				wGradeSkin = 16; //so try 
				break;
			case IE_NEW8:
				wGradeSkin = 17; //so try 
				break;
						case IE_NEW9:
				wGradeSkin = 18; //so try 
				break;
						case IE_NEW10:
				wGradeSkin = 19;
				break;
				...
				wGradeSkin = 21;
				break;//so try 
//das war der fehler  b.r.eak fehlte ah ok dann compil ich ma
			default: //default tritt in allen andren fällen ein, also über 5 = farbe 0, sprich ncihts  (ajloso der effekt)
				wGradeSkin =0;//alle anderen werden jetzt ne 12, mal gucken
			}
		}
...
BOOL CTClientItem::CheckValid()
{
	...
	TRACE("°æ°í:ÁÖ¾îÁø ¾ÆÀÌµð(%u)¿¡ ÇØ´çÇÏ´Â ¾ÆÀÌÅÛÀ» Ã£À» ¼ö ¾ø½À´Ï´Ù.\n", m_wItemID);
	return FALSE;
}
// ====================================================================================================
...
void CTClientItem::CalcAttrID()
{
	...
	m_wAttrID += CTChart::m_vTITEMGRADE[ GetGrade() ].m_bGrade + GetGemy();
...
BYTE bGem = 100;
	...
	for(; itrMag!=endMag; ++itrMag)
	{
		...
		else
		{
			...
			case MTYPE_MAP	:
				m_bApplyMAP = TRUE;
				m_itemAttr.m_wMinMAP;
				m_itemAttr.m_wMaxMAP;
				break;
...
void CTClientItem::SetGems( BYTE bGemy )
{
	m_bGemy = bGemy;
   //Update();
	NotifyUpdate();
