...
static CPoint MIN = 5;
	...
	else
	{
		for( INT i=0 ; i < TWORLDMAP_COMP_COUNT ; ++i )
		{
			...
			if( pCOMP )
			{
				...
				vCOMP_OFFSET[i].x = pt.x-4;
				vCOMP_OFFSET[i].y = pt.y - 28;
			}
		}
	...
	//m_pTALLWORLDMAP2->GetComponentPos(&pt);
...
void CTWorldmapDlg::OnMouseMove( UINT nFlags, CPoint pt)
{
	...
	/*	if( m_pTALLWORLDMAP2)
		{
			m_pTALLWORLDMAP2->GetComponentRect( &rt );
			m_pTALLWORLDMAP2->ComponentToScreen( &rt );
		}
*/
		if( rt.PtInRect( pt ))
		{
...
void CTWorldmapDlg::RenderWORLDMAP( DWORD dwTickCount)
{
	...
	//m_pTALLWORLDMAP2->GetComponentSize( &sizeFRAME);
	FLOAT fLength = ((FLOAT) m_pTMAP->m_pMAP->m_nUnitLength * m_pTMAP->m_pMAP->m_nTileLength);
	FLOAT fMinX = fLength / (2.0f * m_fTSCALE);
	FLOAT fMaxX = fLength - fMinX;
...
//m_pTALLWORLDMAP2->GetComponentRect(&rtBack);
		//m_pTALLWORLDMAP2->ComponentToScreen( &rtBack );
