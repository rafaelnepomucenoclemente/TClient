	...
	case POPUP_CHATTYPE:
		{
			...
			m_vPopup[ePopup]->MoveComponent( CPoint(pt.x, pt.y-sz.cy-120) );
		}
		break;
...
//	m_TAB1 = (CTChatTabButton*)FindKid(ID_CTRLINST_CHAT_TAB2);
//	m_TAB1->ShowComponent(FALSE);
	/*
for( INT i=0; i<TCHAT_SET_COUNT; ++i)
	{
TComponent* pTabTemplate = FindKid( dwChatTab[i] );
pTabTemplate->ShowComponent(bShow);
	}
	*/
...
void CTChatFrame::ShowAllPopup()
{
	for( INT i=0; i<POPUP_COUNT; ++i )
	{
		if( m_vPopup[i] )
			m_vPopup[i]->ShowComponent(TRUE);
	}
}
// -------------------------------------------------------------------------------
...
{
//	m_Jab = (CTChatTabButton*)FindKid(ID_CTRLINST_CHAT_TAB1);
//	m_Jab->ShowComponent(FALSE);
	m_pChatEditCtrl->ShowComponent(bEnable);
	m_vTabs[2]->ShowComponent(FALSE);
	for(INT i=0; i<TCHAT_SET_COUNT; ++i)
	{
		m_vTabs[i]->ShowComponent(bEnable);
