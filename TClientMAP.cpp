...
LPTEXTURESET CTClientMAP::m_pPortal = NULL;
LPTEXTURESET CTClientMAP::m_pChaos = NULL;
								 ...
								 CTClientCAM *pCamera)
{
	...
	for( i=0; i<TDRAWCELL_COUNT; i++)
	{
		...
		for( j=0; j<m_vTNPC[i].m_wCount; j++)
		{
			...
			if(pNPC->m_bNPCType == 6||pNPC->m_bNPCType == 8)
			{
				pTPOS = m_pPortal;
			}
			if(pNPC->m_bNPCType == 3 && pHost->m_bGhost == 1)
			{
				pTPOS = m_pChaos;
			}
								   ...
								   DWORD dwTick)
{
	...
	{
		MessageBox(NULL, "Whatever error!","0",0);
	}
}
