...
//m_pSaddleName->CreateFrom(pDesc);
...
HRESULT CTPetManageDlg::Render(DWORD dwTickCount)
{
	...
	if( !IsPetEmpty() )
	{
		...
		for( INT i=0 ; i < TPET_ITEM_PER_PAGE ; ++i )
		{
			...
			/*	LPTITEM pTITEM = CTChart::FindTITEMTEMP( pPET->wSaddleID );
		
				LPTITEMVISUAL pTVisual = CTChart::FindTITEMVISUAL( pTITEM->m_wVisual[0] );
				m_pSaddle->SetCurImage( pTVisual->m_wIcon );	
m_pSaddleName->m_strText = pTITEM->m_strNAME;
*/


//m_pTime->m_strText = "364 dní";

			LPTITEM pTITEM = CTChart::FindTITEMTEMP( 7503 );
			m_pSpeed->m_strText = "Rychlost : +176%";
			LPTITEMVISUAL pTVisual = CTChart::FindTITEMVISUAL( pTITEM->m_wVisual[0] );
			m_pSaddle->SetCurImage( pTVisual->m_wIcon );
			m_pSaddleName->m_strText = "Sedlo Hbitosti";
...
// ======================================================================
void CTPetManageDlg::UpdateSaddleTime(__int64 ldwTime)
{
			CString strSADDLE;
			...
			else if( SDate > CTClientApp::m_dCurDate)
			{
				CTimeSpan SimeSpan = SDate - CTClientApp::m_dCurDate;

				if( SimeSpan.GetDays() > 0 )
					strSADDLE = CTChart::Format( TSTR_FMT_PET_EXTPERIOD, SimeSpan.GetDays() );
				else
				{
					if( SimeSpan.GetHours() > 0 )
						strSADDLE = CTChart::Format( TSTR_FMT_PET_EXTPERIOD_HOUR_MIN, SimeSpan.GetHours(), SimeSpan.GetMinutes() );
					else
						strSADDLE = CTChart::Format( TSTR_FMT_PET_EXTPERIOD_MIN, SimeSpan.GetMinutes() );
				}
			}
			else
				strSADDLE = CTChart::LoadString( TSTR_FMT_PET_PERIOD_FIRED);
			
			m_pTime->m_strText = strSADDLE;
}
